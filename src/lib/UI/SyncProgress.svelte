...
{#if status === "syncing" || status === "rate_limited"}
    ...
    <div
        bind:this={widgetEl}
        class="fixed z-50 min-w-[200px] cursor-move select-none rounded-lg border border-darkborderc bg-darkbg p-3 shadow-lg"
        style={posX === null
    ...
    >
        {#if status === "rate_limited" && rateLimitInfo}
            ...
            <div class="bg-darkbg2 h-1.5 w-full rounded-full">
                <div class="h-1.5 rounded-full bg-yellow-500 transition-all duration-200" style="width: 100%"></div>
            </div>
        {:else}
            <div class="mb-2 flex items-center gap-2">
            ...
            {#if progress}
                ...
                <div class="bg-darkbg2 h-1.5 w-full rounded-full">
                    <div
                        class="h-1.5 rounded-full bg-green-500 transition-all duration-200"
                        style="width: {progress.total > 0 ? (progress.current / progress.total) * 100 : 0}%"
                    ></div>
                </div>
            {:else}
                <div class="text-xs text-textcolor2">Preparing...</div>
            {/if}
        {/if}
    </div>
{/if}
