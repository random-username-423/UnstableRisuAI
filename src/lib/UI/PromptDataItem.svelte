<script lang="ts">
    ...
    let { promptItem = $bindable(), onRemove = () => {}, moveUp = () => {}, moveDown = () => {} }: Props = $props()
...
<div
    class="flex flex-col rounded-md border border-selected bg-darkbg p-4"
    ondragover={(e) => {
        e.preventDefault()
    }}
    ondrop={(e) => {
