...
<div
    class={"n-scroll relative z-20 rounded-md border border-darkborderc bg-transparent text-textcolor shadow-sm transition-colors duration-200 focus-within:z-40 focus-within:border-borderc focus-within:outline-none focus-within:ring-2 focus-within:ring-borderc" +
        (className ? " " + className : "")}
...
>
    {#if !highlight || RenderState.disableHighlight}
        <textarea
            class="absolute left-0 top-0 z-50 h-full w-full resize-none overflow-y-auto bg-transparent focus-within:outline-none"
            class:px-4={padding}
            class:py-2={padding}
            {autocomplete}
            {placeholder}
    ...
    {:else}
        <div
            class="absolute left-0 top-0 z-50 h-full w-full resize-none overflow-y-auto whitespace-pre-wrap break-words bg-transparent px-4 py-2 focus-within:outline-none"
    ...
    {/if}
    <div class="z-100 absolute hidden flex-col border border-darkborderc bg-bgcolor p-2" bind:this={autoCompleteDom}>
        {#each autocompleteContents as content, i}
            <button
                class="w-full bg-bgcolor px-2 py-1 text-left"
