        ...
        try {
            ...
            for (const file of files) {
                ...
                if (name.endsWith('.fbx')) {
                    fbxFile = file;
                } else if (name.endsWith('.png') || name.endsWith('.jpg') || name.endsWith('.jpeg') || name.endsWith('.tga') || name.endsWith('.bmp')) {
...
<h2 class="text-4xl text-textcolor my-6 font-black relative">3D Viewer</h2>
...
{#if modelName}
    <p class="text-textcolor2 mb-2">Model: {modelName} {#if textureCount > 0}({textureCount} textures){/if}</p>
{/if}
...
<p class="text-textcolor2 text-sm mt-2">
    Controls: Left click + drag to rotate, scroll to zoom, right click + drag to pan
</p>
<p class="text-textcolor2 text-sm">
    Tip: Select FBX file together with texture images (PNG, JPG, TGA, BMP)
</p>
