<script lang="ts">
    ...
    function getStatusText(status: SyncStatus): string {
        switch (status) {
            case "idle":
                return language.syncStatusIdle || "Idle"
            case "syncing":
                return language.syncStatusSyncing || "Syncing..."
            case "error":
                return language.syncStatusError || "Error"
            case "conflict":
                return language.syncStatusConflict || "Conflict Detected"
            case "rate_limited":
            ...
            if (isTauri && googleAuth) {
                ...
                if (response.accessToken) {
                    ...
                    const expiresIn = response.expiresAt
                        ...
                        : undefined
                    syncManager.setTokens(response.accessToken, response.refreshToken, expiresIn)
                    syncManager.enable()
...
<h2 class="mb-2 mt-2 text-2xl font-bold">{language.account} & {language.files}</h2>
...
<div class="mb-2 mt-2 flex flex-col items-start rounded-md bg-darkbg p-3">
    <div class="w-full">
        <h1 class="min-w-0 text-3xl font-black">
            Risu Account{#if DBState.db.account}
                <button
                    class="float-right rounded-md bg-selected p-1 text-sm font-light transition-colors hover:bg-green-500"
                    onclick={async () => {
    ...
    {:else}
        ...
        <button
            class="mt-2 rounded-md bg-selected p-2 transition-colors hover:bg-green-500"
            onclick={() => {
...
{#if openIframe}
    <div class="fixed left-0 top-0 flex h-full w-full items-center justify-center bg-black bg-opacity-50">
        <iframe src={openIframeURL} title="login" class="h-full w-full"> </iframe>
    </div>
{/if}
