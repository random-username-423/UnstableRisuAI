<script lang="ts">
    ...
    import { changeFullscreen, selectSingleFile } from "src/ts/utils/util"
...
{#if submenu === 0 || submenu === -1}
    <span class="mt-4 text-textcolor">{language.theme}</span>
    ...
    {#if DBState.db.theme === "customHTML"}
        <span class="mt-4 text-textcolor">{language.chatHTML} <Help key="chatHTML" /></span>
        <TextAreaInput bind:value={DBState.db.guiHTML} />
    {/if}

    {#if DBState.db.theme === "waifu"}
        <span class="mt-4 text-textcolor">{language.waifuWidth}</span>
        <SliderInput min={50} max={200} bind:value={DBState.db.waifuWidth} />
        <span class="text-sm text-textcolor2">{DBState.db.waifuWidth}%</span>

        <span class="mt-4 text-textcolor">{language.waifuWidth2}</span>
        <SliderInput min={20} max={150} bind:value={DBState.db.waifuWidth2} />
        <span class="text-sm text-textcolor2">{DBState.db.waifuWidth2}%</span>
    {/if}

    <span class="mt-4 text-textcolor">{language.colorScheme}</span>
    ...
    {#if DBState.db.colorSchemeName === "custom"}
        <div class="m-2 rounded-md border border-darkborderc p-2">
    ...
    <span class="mt-4 text-textcolor">{language.textColor}</span>
    ...
    {#if DBState.db.textTheme === "custom"}
        ...
        <div class="mt-2 flex items-center">
            <ColorInput
                nullable
                bind:value={DBState.db.customTextTheme.FontColorQuote1}
        ...
        <div class="mt-2 flex items-center">
            <ColorInput
                nullable
                bind:value={DBState.db.customTextTheme.FontColorQuote2}
                oninput={updateTextThemeAndCSS}
            />
    ...
    <span class="mt-4 text-textcolor">{language.font}</span>
...
{#if submenu === 1 || submenu === -1}
    <span class="mt-4 text-textcolor">{language.UISize}</span>
    ...
    <SliderInput
        min={-5}
        max={5}
        bind:value={DBState.db.textAreaSize}
        onchange={updateGuisize}
        customText={guiSizeText(DBState.db.textAreaSize)}
        marginBottom
    />
    ...
    <SliderInput
        min={0}
        max={3}
        bind:value={DBState.db.textAreaTextSize}
        onchange={updateGuisize}
        customText={guiSizeText(DBState.db.textAreaTextSize)}
        marginBottom
    />
    ...
    <SliderInput
        min={0}
        max={3}
        bind:value={DBState.db.sideBarSize}
        onchange={updateGuisize}
        customText={guiSizeText(DBState.db.sideBarSize)}
        marginBottom
    />
    ...
    <SliderInput
        ...
        max={40}
        step={1}
        bind:value={DBState.db.assetWidth}
        customText={DBState.db.assetWidth === -1
            ? "Unlimited"
            : DBState.db.assetWidth === 0
              ? "Hidden"
              : `${DBState.db.assetWidth.toFixed(1)} rem`}
        marginBottom
    />
    ...
    <SliderInput
        ...
        step={0.05}
        fixed={2}
        bind:value={DBState.db.animationSpeed}
...
{#if submenu === 2 || submenu === -1}
    ...
    {#if DBState.db.textScreenColor}
        <div class="mt-2 flex items-center">
            <Check
                ...
                hiddenName
            />
            <input type="color" class="style2 mr-2 text-sm" bind:value={DBState.db.textScreenColor} />
    ...
    {#if DBState.db.textScreenBorder}
        <div class="mt-2 flex items-center">
            <Check
                ...
                hiddenName
            />
            <input type="color" class="style2 mr-2 text-sm" bind:value={DBState.db.textScreenBorder} />
    ...
    {#if DBState.db.customQuotes}
        <span class="mt-4 text-textcolor">{language.leadingDoubleQuote}</span>
        <TextInput bind:value={DBState.db.customQuotesData[0]} />

        <span class="mt-4 text-textcolor">{language.trailingDoubleQuote}</span>
        <TextInput bind:value={DBState.db.customQuotesData[1]} />

        <span class="mt-4 text-textcolor">{language.leadingSingleQuote}</span>
        <TextInput bind:value={DBState.db.customQuotesData[2]} />

        <span class="mt-4 text-textcolor">{language.trailingSingleQuote}</span>
    ...
    <span class="mt-4 text-textcolor">{language.customCSS}<Help key="customCSS" /></span>
    <TextAreaInput
        bind:value={DBState.db.customCSS}
        onInput={() => {
            updateTextThemeAndCSS()
        }}
    />
