...
<h2 class="mb-2 mt-2 text-2xl font-bold">{language.language}</h2>

<span class="mt-4 text-textcolor">{language.UiLanguage}</span>
<SelectInput
    className="mt-2"
    bind:value={DBState.db.language}
    onchange={async () => {
...
{/if}
<span class="mt-4 text-textcolor">{language.translatorLanguage}</span>
...
{#if DBState.db.translator}
    <span class="mt-4 text-textcolor">{language.translatorType}</span>
    ...
    {#if DBState.db.translatorType === "deepl"}
        {#if !isTauri}
            <span class="ml-2 text-xs text-draculared">{language.webdeeplwarn}</span>
        {/if}
        <span class="mt-4 text-textcolor">{language.deeplKey}</span>
    ...
    {#if DBState.db.translatorType === "deeplX"}
        <span class="mt-4 text-textcolor" placeholder="http://localhost:1188">{language.deeplXUrl}</span>
        <TextInput bind:value={DBState.db.deeplXOptions.url} />

        <span class="mt-4 text-textcolor">{language.deeplXToken}</span>
        <TextInput bind:value={DBState.db.deeplXOptions.token} />
    {/if}

    {#if DBState.db.translatorType === "llm"}
        <span class="mt-4 text-textcolor">{language.translationResponseSize}</span>
        <NumberInput min={0} max={2048} marginBottom={true} bind:value={DBState.db.translatorMaxResponse} />
        <span class="mt-4 text-textcolor">{language.translatorPrompt} <Help key="translatorPrompt" /></span>

        <TextAreaInput
            bind:value={DBState.db.translatorPrompt}
    ...
    {#if DBState.db.translatorType === "google"}
        <span class="mt-4 text-textcolor">{language.sourceLanguage}</span>
