...
<h2 class="mb-2 mt-2 text-2xl font-bold">{language.otherBots}</h2>
    ...
    <Arcodion name={language.imageGeneration} styled disabled={submenu !== -1}>
        ...
        {#if DBState.db.sdProvider === "webui"}
            ...
            <span class="mb-2 text-xs text-draculared"
                >You must use WebUI without agpl license or use unmodified version with agpl license to observe the
                contents of the agpl license.</span
            >
            {#if !isTauri}
                <span class="mb-2 text-xs text-draculared"
        ...
        {#if DBState.db.sdProvider === "novelai"}
                ...
                <div class="relative">
                    ...
                    >
                        {#if !DBState.db.NAIImgConfig.vibe_data || !DBState.db.NAIImgConfig.vibe_data.thumbnail}
                            <div
                                class="flex h-20 w-20 cursor-pointer items-center justify-center rounded-md bg-textcolor2 shadow-lg hover:text-green-500"
                            >
                        ...
                        {:else}
                            <img
                                ...
                                alt="Vibe Preview"
                                class="h-40 cursor-pointer rounded-md bg-textcolor2 shadow-lg hover:text-green-500"
                            />
                        {/if}
                    </button>
                    ...
                    {#if DBState.db.NAIImgConfig.vibe_data}
                        <button
                            ...
                            class="absolute right-2 top-2 rounded bg-red-500 px-2 py-1 font-bold text-white hover:bg-red-700"
                        >
                            Delete
                        </button>
                    {/if}
            ...
            {#if DBState.db.NAIImgConfig.reference_mode === "character" && (DBState.db.NAIImgModel === "nai-diffusion-4-5-full" || DBState.db.NAIImgModel === "nai-diffusion-4-5-curated")}
                ...
                <span class="mb-2 block text-xs text-textcolor2">Leave blank to use the character's default image.</span
                >
            ...
            {#if DBState.db.NAII2I}
                ...
                <span class="block text-xs text-textcolor2">Leave blank to use the character's default image.</span>

                <span class="mt-2 text-textcolor">Strength</span>
                <SliderInput min={0} max={0.99} step={0.01} fixed={2} bind:value={DBState.db.NAIImgConfig.strength} />
                <span class="mt-2 text-textcolor">Noise</span>
        ...
        {#if DBState.db.sdProvider === "Imagen"}
            <span class="mt-2 text-textcolor">GoogleAI API Key</span>
            <TextInput
                marginBottom={true}
                size="sm"
                placeholder="..."
...
{#if submenu === 0 || submenu === -1}
    <Arcodion name={language.longTermMemory} styled disabled={submenu !== -1}>
        ...
        {#if DBState.db.hanuraiEnable}
            <span class="mb-2 max-w-full text-wrap break-words text-sm text-textcolor2">{language.hanuraiDesc}</span>
        ...
        {:else if DBState.db.hypav2}
            <span class="mb-2 max-w-full text-wrap break-words text-sm text-textcolor2">{language.hypaV2Desc}</span>
            ...
            <TextAreaInput
                size="sm"
                bind:value={DBState.db.supaMemoryPrompt}
        ...
        {:else if DBState.db.hypaV3}
            <span class="mb-6 max-w-full text-wrap break-words text-sm text-textcolor2"
                >{language.hypaV3Settings.descriptionLabel}</span
            >
            <span class="text-textcolor">Preset</span>
            <select
                class="text-md mb-1 rounded-md border border-darkborderc bg-transparent px-4 py-2 text-textcolor shadow-sm transition-colors duration-200 focus:border-borderc focus:outline-none focus:ring-2 focus:ring-borderc"
            ...
            <div class="mb-8 flex items-center">
                ...
                <div class="ml-2 mr-4 h-full w-px bg-darkborderc"></div>
        ...
        {:else if DBState.db.supaModelType !== "none" && DBState.db.hypav2 === false && DBState.db.hypaV3 === false}
            <span class="mb-2 max-w-full text-wrap break-words text-sm text-textcolor2">{language.supaDesc}</span>
            <span class="mt-4 text-textcolor">{language.SuperMemory} {language.model}</span>
            <SelectInput className="mt-2 mb-2" bind:value={DBState.db.supaModelType}>
                <OptionInput value="distilbart">distilbart-cnn-6-6 (Free/Local)</OptionInput>
                <OptionInput value="instruct35">OpenAI 3.5 Turbo Instruct</OptionInput>
