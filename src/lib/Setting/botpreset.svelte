<script lang="ts">
    ...
    import type {
        ...
        PromptItemChat,
    } from "src/ts/process/utils/prompt"
    ...
    async function checkDiff(prompt1: string, prompt2: string): Promise<void> {
        ...
        if (lineDiffs.length === 1 && !lineDiffs[0].added && !lineDiffs[0].removed) {
            const userResponse = await alertConfirm(
                "The two prompts are identical. Would you like to review the content?"
            )

            if (userResponse) {
                resultHtml =
                    `<div style="background-color: #4caf50; color: white; padding: 10px 20px; border-radius: 5px; text-align: center;">No differences detected.</div>` +
                    resultHtml
                alertMd(resultHtml)
            }
        } else {
            ...
            resultHtml =
                `<div id="differences-detected" style="background-color: #ff9800; color: white; padding: 10px 20px; border-radius: 5px; text-align: center;">Differences detected. Please review the changes.${diffCounts}</div>` +
                resultHtml
            alertMd(resultHtml)
    ...
    function differencesTooltip(changedLines: string[]) {
        ...
        differencesDetected.addEventListener("mouseenter", () => {
            const tooltipContent = !changedLines.length
                ? ""
                : `<div><strong>Changed Lines</strong></div>
...
<div class="absolute z-40 flex h-full w-full items-center justify-center bg-black bg-opacity-50">
    <div
        class="break-any flex max-h-full w-124 max-w-3xl flex-col overflow-y-auto rounded-md bg-darkbg p-4"
    ...
    >
        ...
        {#each DBState.db.botPresets as preset, i}
            <div
                class="h-1 w-full transition-colors duration-200 hover:bg-gray-600"
                role="listitem"
                ondragover={(e) => {
                    e.preventDefault()
            ...
            >
                ...
                {:else}
                    {#if i < 9}
                        <span class="mr-2 w-2 text-center text-textcolor2">{i + 1}</span>
                    {/if}
                    {#if preset.image}
                        <img
                            src={preset.image}
                            alt="icon"
                            class="mr-2 h-6 min-h-6 w-6 min-w-6 rounded-md"
                            decoding="async"
                        />
                    {/if}
        ...
        <span class="text-sm text-textcolor2">{language.quickPreset}</span>
    </div>
</div>
