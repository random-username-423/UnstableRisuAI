...
>
    ...
    {:else}
        <div
            class="default-chat-screen relative flex h-full w-full flex-col-reverse overflow-y-auto"
            onscroll={(e) => {
                ...
                {#if DBState.db.characters[ChatState.selectedCharId].chats[DBState.db.characters[ChatState.selectedCharId].chatPage].message?.length <= loadPages}
                    {#if DBState.db.characters[ChatState.selectedCharId].type !== "group"}
                        <Chat
                            ...
                            isLastMemory={false}
                            currentPage={(DBState.db.characters[ChatState.selectedCharId].chats[
                                DBState.db.characters[ChatState.selectedCharId].chatPage
                            ].fmIndex ?? -1) + 2}
