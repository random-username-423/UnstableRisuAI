    ...
    function createDecorations(doc) {
        ...
        for (const markdown of markdowns) {
            ...
            for (const match of text.matchAll(markdown.regex)) {
                ...
                if (from < to) {
                    decorations.push(Decoration.mark({ class: markdown.className }).range(from, to))
                }
            }
        }
...
<div class="ml-2 mr-2 flex flex-1 items-end">
    {#if DBState.db.useAutoTranslateInput}
        <button
            onclick={toggleTranslateText}
            class="mr-2 flex h-12 w-12 items-center justify-center rounded-md bg-textcolor2 text-gray-100 transition-colors hover:bg-green-500"
        >
            {#if toggleTranslate}
                <LanguagesIcon />
            {:else}
