<script lang="ts">
    ...
    } from "src/ts/process/scripting/triggers"
                    ...
                    setTimeout(() => {
                        try {
                            if (value && value.length > selectedTriggerIndex) {
                                ...
                                if (
                                    selectedEffectIndexSaved >= 0 &&
                                    value[selectedTriggerIndex]?.effect &&
                                    selectedEffectIndexSaved < value[selectedTriggerIndex].effect.length
                                ) {
    ...
    const deleteEffect = () => {
        ...
        if (
            type.type === "v2If" ||
            type.type === "v2IfAdvanced" ||
            type.type === "v2Loop" ||
            type.type === "v2Else" ||
            type.type === "v2LoopNTimes"
        ) {
            let pointer = selectedEffectIndex
            let indent = (type as triggerEffectV2).indent
            while (pointer < value[selectedIndex].effect.length) {
                if (
                    value[selectedIndex].effect[pointer].type === "v2EndIndent" &&
                    (value[selectedIndex].effect[pointer] as triggerEffectV2).indent === indent + 1
                ) {
    ...
    const getInsertIndent = (insertIndex: number): number => {
        ...
        if (
            prevEffect &&
            (prevEffect.type === "v2If" ||
                prevEffect.type === "v2IfAdvanced" ||
                prevEffect.type === "v2Loop" ||
                prevEffect.type === "v2LoopNTimes")
        ) {
    ...
    const getBlockRange = (startIndex: number): { start: number; end: number } => {
        ...
        if (
            !startEffect ||
            (startEffect.type !== "v2If" &&
                startEffect.type !== "v2IfAdvanced" &&
                startEffect.type !== "v2Loop" &&
                startEffect.type !== "v2LoopNTimes")
        ) {
            return { start: startIndex, end: startIndex }
        }
    ...
    const canMoveEffect = (fromIndex: number, toIndex: number): boolean => {
        ...
        if (fromIndex >= value[selectedIndex].effect.length || toIndex > value[selectedIndex].effect.length)
            return false
        ...
        if (
            fromEffect.type === "v2If" ||
            fromEffect.type === "v2IfAdvanced" ||
            fromEffect.type === "v2Loop" ||
            fromEffect.type === "v2LoopNTimes"
        ) {
            ...
            if (e.key === "ArrowUp") {
                if (selectedEffectIndex > 0) {
                    ...
                    if (e.altKey) {
                        ...
                        if (
                            before.type !== "v2EndIndent" &&
                            after.type !== "v2EndIndent" &&
                            before.type !== "v2If" &&
                            after.type !== "v2If" &&
                            before.type !== "v2IfAdvanced" &&
                            after.type !== "v2IfAdvanced" &&
                            before.type !== "v2Loop" &&
                            after.type !== "v2Loop" &&
                            before.type !== "v2LoopNTimes" &&
                            after.type !== "v2LoopNTimes" &&
...
<div class="contain mt-2 flex w-full max-w-full flex-col rounded-md border-1 border-selected bg-darkbg">
    <Button
        onclick={(e) => {
            e.stopPropagation()
            menuMode = 0
...
{#if selectedIndex > 0}
    <Portal>
        ...
        <div
            class="absolute bottom-0 top-0 z-40 flex h-full w-full max-w-full items-center justify-center bg-black bg-opacity-50 text-textcolor"
            onclick={(e) => {
        ...
        >
            {#if contextMenu}
                <div
                    class="absolute z-50 flex w-28 flex-col gap-2 rounded-md border border-darkborderc bg-darkbg p-2"
                    style={contextMenuLoc.style}
                    onclick={(e) => {
                        e.stopPropagation()
                    }}
                    oncontextmenu={(e) => {
                ...
                >
                    {#if selectedTriggerIndices.length > 1 && selectMode === 0}
                        <div class="mb-2 border-b border-darkborderc pb-2 text-xs text-textcolor2">
            ...
            {/if}
            <div
                class="flex max-h-full max-w-full flex-col-reverse overflow-y-auto border border-darkborderc bg-bgcolor p-2 md:flex-row md:overflow-y-visible"
            ...
            >
                {#if menuMode === 0}
                    <div class="mt-2 flex flex-col pr-2 md:mt-0 md:h-full md:w-96">
                        <div
                        ...
                        >
                            {#each value as trigger, i}
                                ...
                                {:else}
                                    <div
                                        class="h-0.5 min-h-0.5 w-full transition-all duration-200"
                                    ...
                                    <button
                                        class="trigger-item select-none p-2 text-start"
                                        ...
                                        ondragstart={(e) => {
                                            ...
                                            dragElement.className =
                                                "absolute -top-96 -left-96 px-4 py-2 bg-darkbg text-textcolor2 rounded text-sm whitespace-nowrap shadow-lg pointer-events-none z-50"
                        ...
                        <div class="flex gap-2">
                            <button
                                class="border-t-darkborderc p-2 text-start text-textcolor2 hover:text-textcolor focus:bg-bgcolor"
                                onclick={() => {
                                    value.push({
                                        comment: "",
                                        type: "manual",
                    ...
                    <div class="flex h-svh min-h-svh flex-col bg-darkbg md:h-auto md:min-h-0 md:flex-1">
                        <div
                            class="mb-0 grid grid-cols-2 items-stretch"
                            onclick={() => {
                                selectMode = 1
                                selectedEffectIndex = -1
                            }}
                        >
                            <div class="flex h-full min-h-[4.5rem] flex-col p-2">
                                <span class="block text-textcolor2">{language.name}</span>
                                <div class="flex min-h-[2.5rem] flex-1">
                                    <TextInput
                                        className="flex-1"
                        ...
                        <div
                            class="relative mx-2 mb-2 flex-1 overflow-y-auto overflow-x-hidden rounded-md border border-darkborderc"
                        ...
                        >
                            ...
                            {#each value && value[selectedIndex] && value[selectedIndex].effect ? value[selectedIndex].effect : [] as effect, i}
                                ...
                                >
                                    <button
                                        class="relative flex-1 overflow-hidden whitespace-normal break-all p-2 text-start text-purple-500"
                                    ...
                                    {#if effect.type !== "v2EndIndent" && effect.type !== "v2Else"}
                                        <div
                                            class="flex h-full w-8 cursor-move items-center justify-center opacity-30 transition-opacity hover:opacity-70"
                                            draggable={!isMobileScreen}
                                            onclick={(e) => {
                                                e.stopPropagation()
                                            }}
                                        ...
                                        >
                                            <div class="select-none text-xs text-textcolor2">⋮⋮</div>
                                        </div>
                                    {:else}
                                        <div class="flex h-full w-8 items-center justify-center">
                                            <div class="select-none text-xs text-textcolor2 opacity-20"></div>
                                        </div>
                                    {/if}
                                </div>
                            {/each}
                            ...
                            <div
                                class="h-0.5 min-h-0.5 w-full transition-all duration-200"
                                class:hover:bg-gray-600={!isMobileScreen && !isEffectDragging}
                                class:h-0.5={!isEffectDragging ||
                                    effectDragOverIndex !==
                                        (value && value[selectedIndex] && value[selectedIndex].effect
                                            ? value[selectedIndex].effect.length
                                            : 0)}
                                class:h-1={isEffectDragging &&
                                    effectDragOverIndex ===
                                        (value && value[selectedIndex] && value[selectedIndex].effect
                                            ? value[selectedIndex].effect.length
                                            : 0)}
                                class:bg-blue-500={isEffectDragging &&
                                    effectDragOverIndex ===
                                        (value && value[selectedIndex] && value[selectedIndex].effect
                                            ? value[selectedIndex].effect.length
                                            : 0)}
                                class:shadow-lg={isEffectDragging &&
                                    effectDragOverIndex ===
                                        (value && value[selectedIndex] && value[selectedIndex].effect
                                            ? value[selectedIndex].effect.length
                                            : 0)}
                            ...
                            <button
                                class="w-full p-2 text-start hover:bg-selected"
                                onclick={() => {
                                    //add effect
                ...
                {:else if menuMode === 1}
                    <div class="flex flex-1 flex-col overflow-y-auto bg-darkbg md:flex-row md:overflow-y-visible">
                        <div class="flex w-full flex-col border-b border-darkborderc md:w-48 md:border-b-0 md:border-r">
                            <div class="flex min-h-16 items-center border-b border-darkborderc p-4">
                                <button
                                ...
                                <h3 class="ml-4 text-lg font-medium text-textcolor md:hidden">
                                    {language.triggerCategories[selectedCategory] || selectedCategory}
                                </h3>
                            </div>
                            <div class="hidden md:flex md:flex-1 md:flex-col md:overflow-y-auto">
                                {#each getAvailableCategories() as category}
                                    <button
                                        class="w-full p-3 text-left transition-colors hover:bg-selected hover:text-textcolor"
                            ...
                            <div class="border-b border-darkborderc p-4 md:hidden">
                            ...
                            <div class="hidden border-t border-darkborderc p-4 md:block">
                        ...
                        <div class="flex flex-1 flex-col overflow-y-auto">
                            <div class="hidden min-h-16 items-center border-b border-darkborderc p-4 md:flex">
                                <h3 class="text-lg font-medium text-textcolor">
                                    {language.triggerCategories[selectedCategory] || selectedCategory}
                                </h3>
                            </div>
                            <div class="flex-1 overflow-y-auto">
                                {#each getFilteredTriggers() as type}
                                    <button
                                        class="w-full p-3 text-left text-textcolor2 transition-colors hover:bg-selected hover:text-textcolor"
                            ...
                            <div class="border-t border-darkborderc p-4 md:hidden">
                                <div class="text-xs text-textcolor2">
                                    <CheckInput
                                        bind:check={DBState.db.showDeprecatedTriggerV2}
                                        name={language.showDeprecatedTriggerV2}
                ...
                {:else if menuMode === 2 || menuMode === 3}
                    <div class="flex flex-1 flex-col overflow-y-auto">
                        <div class="mb-4 flex items-center gap-2">
                            <button
                                class="border-t-darkborderc p-2 text-start text-textcolor2 hover:text-textcolor"
                                onclick={() => {
                        ...
                        {:else if editTrigger.type === "v2If" || editTrigger.type === "v2IfAdvanced"}
                            ...
                            <span class="text block text-textcolor">{language.triggerInputLabels.condition}</span>
                            <SelectInput
                                bind:value={editTrigger.condition}
                                onchange={(e) => {
                        ...
                        {:else if editTrigger.type === "v2Random"}
                            <span class="text block text-textcolor">{language.triggerInputLabels.min}</span>
                            <SelectInput bind:value={editTrigger.minType}>
                                <OptionInput value="value">{language.triggerInputLabels.value}</OptionInput>
                                <OptionInput value="var">{language.triggerInputLabels.var}</OptionInput>
                            </SelectInput>
                            <TextInput bind:value={editTrigger.min} />
                            <span class="text block text-textcolor">{language.triggerInputLabels.max}</span>
                            ...
                            <span class="text block text-textcolor">{language.triggerInputLabels.outputVar}</span>
                            <TextInput bind:value={editTrigger.outputVar} />
                            ...
                            onclick={() => {
                                ...
                                if (selectedEffectIndex === -1) {
                                    ...
                                    if (
                                        ...
                                        editTrigger.type === "v2Loop" ||
                                        editTrigger.type === "v2Else" ||
                                        editTrigger.type === "v2LoopNTimes"
                                    ) {
                                    ...
                                    if (isIfType) {
                                        ...
                                        if (addElse && !hasExistingElse) {
                                            ...
                                            if (elseEndIndex !== -1) {
                                                value[selectedIndex].effect.splice(
                                                    elseIndex,
                                                    elseEndIndex - elseIndex + 1
                                                )
                                            }
                                        }
                                    }
