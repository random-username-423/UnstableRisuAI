...
>
    ...
    {#if open}
        <div class="seperator p-2">
            ...
            <div class="flex flex-col rounded-md border border-selected px-2 py-4">
                {#if value.conditions.length === 0}
                    <span class="text-sm text-textcolor2">{language.always}</span>
                {/if}
                {#each value.conditions as cond, i}
                    {#if i > 0}
                        <hr class="my-4 border-selected" />
                    {/if}
                    ...
                    {#if cond.type === "exists"}
                        ...
                        <span class="text-sm text-textcolor2">{language.value}</span>
                        <TextAreaInput highlight bind:value={cond.value} />

                        <span class="text-sm text-textcolor2">{language.searchDepth}</span>
                        <NumberInput size="sm" bind:value={cond.depth} />
                    {/if}
                    {#if cond.type === "var" || cond.type === "chatindex" || cond.type === "value"}
                        {#if cond.type === "var"}
                            <span class="text-sm text-textcolor2">{language.varableName}</span>
            ...
            <div class="flex flex-col rounded-md border border-selected px-2 py-4">
                {#if value.effect.length === 0}
                    <span class="text-sm text-textcolor2">{language.noEffect}</span>
                {/if}
                {#each value.effect as effect, i}
                    ...
                    {#if (value.type !== "start" && (effect.type === "systemprompt" || effect.type === "stop")) || (value.type !== "output" && effect.type === "sendAIprompt")}
                        <span class="text-sm text-red-400">{language.invaildTriggerEffect}</span>
                    {/if}
                    {#if !lowLevelAble && (effect.type === "runLLM" || effect.type === "checkSimilarity" || effect.type === "showAlert" || effect.type === "sendAIprompt" || effect.type === "extractRegex" || effect.type === "runImgGen" || effect.type === "runAxLLM")}
                        <span class="text-sm text-red-400">{language.triggerLowLevelOnly}</span>
                    {/if}

                    {#if effect.type === "systemprompt"}
                        <span class="text-sm text-textcolor2">{language.location}</span>
                    ...
                    {#if effect.type === "setvar"}
                        ...
                        <span class="text-sm text-textcolor2">{language.operator}</span>
                    ...
                    {#if effect.type === "runLLM"}
                        <span class="text-sm text-textcolor2">{language.prompt} <Help key="triggerLLMPrompt" /></span>
                        <TextAreaInput highlight bind:value={effect.value} />

                        <span class="text-sm text-textcolor2">{language.resultStoredVar}</span>
                        <TextInput bind:value={effect.inputVar} />
                    {/if}
                    {#if effect.type === "checkSimilarity"}
                        <span class="text-sm text-textcolor2">{language.prompt}</span>
                    ...
                    {#if effect.type === "impersonate"}
                        <span class="text-sm text-textcolor2">{language.role}</span>
                    ...
                    {#if effect.type === "extractRegex"}
                        ...
                        <span class="text-sm text-textcolor2">{language.regex}</span>
                        <TextInput bind:value={effect.regex} />

                        <span class="text-sm text-textcolor2">{language.flags}</span>
                        <TextInput bind:value={effect.flags} />

                        <span class="text-sm text-textcolor2">{language.resultFormat}</span>
                    ...
                    {#if effect.type === "runImgGen"}
                        ...
                        <span class="text-sm text-textcolor2">{language.negPrompt}</span>
                    ...
                    {#if effect.type === "cutchat"}
                        <span class="text-sm text-textcolor2">{language.start}</span>
                        <TextInput bind:value={effect.start} />

                        <span class="text-sm text-textcolor2">{language.end}</span>
                        <TextInput bind:value={effect.end} />
                    {/if}

                    {#if effect.type === "modifychat"}
                        <span class="text-sm text-textcolor2">{language.index}</span>
