<script lang="ts">
    ...
    import { addLorebook, addLorebookFolder, exportLoreBook, importLoreBook } from "src/ts/process/prompt/lorebook.svelte";
    ...
    function isAllChatLoreAlwaysActive() {
        const localLore = DBState.db.characters[ChatState.selectedCharId].chats[DBState.db.characters[ChatState.selectedCharId].chatPage].localLore;
...
{#if !globalMode}
    <div class="flex w-full rounded-md border border-selected">
        <button onclick={() => {
            ...
            <span>{DBState.db.characters[ChatState.selectedCharId].type === 'group' ? language.group : language.character}</span>
        </button>
        <button onclick={() => {
            submenu = 1
...
{#if submenu !== 2}
    {#if !globalMode}
        <div class="text-textcolor2 mt-4 mb-4 text-sm">{submenu === 0 ? DBState.db.characters[ChatState.selectedCharId].type === 'group' ? language.groupLoreInfo : language.globalLoreInfo : language.localLoreInfo}</div>
    {/if}
    <LoreBookList globalMode={globalMode} submenu={submenu} lorePlus={(!globalMode) && DBState.db.characters[ChatState.selectedCharId]?.lorePlus} />
{:else}
    {#if DBState.db.characters[ChatState.selectedCharId].loreSettings}
        ...
        <div class="flex items-center mt-4">
            <Check bind:check={DBState.db.characters[ChatState.selectedCharId].loreSettings.recursiveScanning} name={language.recursiveScanning}/>
        </div>
        <div class="flex items-center mt-4">
            <Check bind:check={DBState.db.characters[ChatState.selectedCharId].loreSettings.fullWordMatching} name={language.fullWordMatching}/>
        </div>
        <span class="text-textcolor mt-4 mb-2">{language.loreBookDepth}</span>
        <NumberInput size="sm" min={0} max={20} bind:value={DBState.db.characters[ChatState.selectedCharId].loreSettings.scanDepth} />
        <span class="text-textcolor">{language.loreBookToken}</span>
        <NumberInput size="sm" min={0} max={4096} bind:value={DBState.db.characters[ChatState.selectedCharId].loreSettings.tokenBudget} />
    {:else}
        <div class="flex items-center mt-4">
    ...
    <div class="flex items-center mt-4">
        {#if DBState.db.useExperimental}
            <Check bind:check={DBState.db.characters[ChatState.selectedCharId].lorePlus}
