...
{#if submenu !== 2}
    {#if !globalMode}
        <div class="text-textcolor2 mt-4 mb-4 text-sm">{submenu === 0 ? DBState.db.characters[$selectedCharID].type === 'group' ? language.groupLoreInfo : language.globalLoreInfo : language.localLoreInfo}</div>
    {/if}
