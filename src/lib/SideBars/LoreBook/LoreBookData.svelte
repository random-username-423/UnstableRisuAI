...
<div
    class={"flex w-full flex-col " +
        (isLastInContainer
            ? "mb-0 border-0 pb-0" // Last item in container: no border
            : "mb-2 border-b border-b-selected pb-2 last:mb-0 last:border-0 last:pb-0")}
    class:no-sort={value.mode === "folder" && openFolders > 0}
    data-risu-idx={idx}
    data-risu-idgroup={idgroup}
>
    <div class="flex w-full items-center p-1 transition-colors">
        {#if value.mode !== "child"}
            <button
                class="endflex valuer flex items-center border-darkborderc"
                onclick={() => {
    ...
    {#if open}
        {#if value.mode === "folder"}
            <div class="mb-2 mt-2 flex w-full flex-col border-0 outline-none">
                <span class="mb-2 mt-6 text-textcolor">{language.folderName}</span>
        ...
        {:else}
            <div class="mb-2 mt-2 flex w-full flex-col border-0 outline-none">
                <span class="mt-6 text-textcolor">{language.name} <Help key="loreName" /></span>
                <TextInput size="sm" bind:value={value.comment} />
                {#if !lorePlus}
                    {#if !value.alwaysActive}
                        <span class="mt-6 text-textcolor"
                        ...
                        {#if value.selective}
                            <span class="mt-6 text-textcolor">{language.SecondaryKeys}</span>
                            <span class="text-xs text-textcolor2">{language.activationKeysInfo}</span>
                ...
                {#if !lorePlus}
                    {#if !(value.activationPercent === undefined || value.activationPercent === null)}
                        <span class="mt-6 text-textcolor">{language.activationProbability}</span>
                        <NumberInput
                            size="sm"
                ...
                {/if}
                {#if !lorePlus}
                    <span class="mt-4 text-textcolor">{language.insertOrder} <Help key="loreorder" /></span>
                    <NumberInput size="sm" bind:value={value.insertorder} min={0} max={1000} />
                {/if}
                <span class="mb-2 mt-4 text-textcolor">{language.prompt}</span>
                <TextAreaInput highlight autocomplete="off" bind:value={value.content} />
                {#await getTokens(value.content)}
                    <span class="mb-2 mt-2 text-sm text-textcolor2">{tokens} {language.tokens}</span>
                {:then e}
                    <span class="mb-2 mt-2 text-sm text-textcolor2">{e} {language.tokens}</span>
                {/await}
