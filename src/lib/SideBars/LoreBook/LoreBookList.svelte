<script lang="ts">
    ...
    const waitForDOMReady = async () => {
        ...
        if (externalLoreBooks) {
            ...
            expectedElements = DBState.db.characters[ChatState.selectedCharId].chats[
            ...
            expectedElements = DBState.db.characters[ChatState.selectedCharId].globalLore.filter(
                (item) => (!showFolder && !item.folder) || showFolder === item.folder
            ).length
        }
...
{#key sorted}
    <div
        class="flex flex-col rounded-md border-1 border-solid border-selected p-2"
        bind:this={ele}
    ...
    >
        ...
        {:else if externalLoreBooks}
            ...
            {/if}
        {:else if submenu === 0}
            {@const visibleItems = DBState.db.characters[ChatState.selectedCharId].globalLore.filter(
                (book) => (!showFolder && !book.folder) || showFolder === book.folder
            )}
            {@const lastVisibleItem = visibleItems[visibleItems.length - 1]}
            ...
            {:else}
                {#each DBState.db.characters[ChatState.selectedCharId].globalLore as book, i}
                    {#if (!showFolder && !book.folder) || showFolder === book.folder}
                        <LoreBookData
                            {idgroup}
                            bind:value={DBState.db.characters[ChatState.selectedCharId].globalLore[i]}
                            idx={i}
                            isOpen={openedRefs.has(book)}
                            openFolders={openFolders()}
                            isLastInContainer={book === lastVisibleItem}
                            ...
                            onClose={(isDetail = true) => onClose(isDetail, book)}
                            {lorePlus}
                            bind:externalLoreBooks={DBState.db.characters[ChatState.selectedCharId].globalLore}
                        />
                    {:else}
                        <!-- Hidden marker for filtered items (for SortableJS) -->
                        <div data-risu-idx={i} data-risu-idgroup={idgroup} style="display: none;"></div>
            ...
            {/if}
        {:else if submenu === 1}
            {@const visibleItems = DBState.db.characters[ChatState.selectedCharId].chats[
                DBState.db.characters[ChatState.selectedCharId].chatPage
