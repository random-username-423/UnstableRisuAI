<script lang="ts">
    ...
    import type { SummaryItemState, ExpandedMessageState, SearchState } from "./types"
    import { alertConfirmTwice, handleDualAction, getFirstMessage, processRegexScript } from "./utils"
...
<div class="flex flex-col rounded-lg border border-zinc-700 bg-zinc-800/50 p-2 sm:p-4">
    ...
    <div class="flex items-center justify-between">
            ...
            {#if filterSelected && hypaV3Data.metrics}
                <div class="flex flex-wrap gap-1">
                    {#if hypaV3Data.metrics.lastImportantSummaries.includes(summaryIndex)}
                        <span
                            class="whitespace-nowrap rounded-full bg-purple-900/70 px-1.5 py-0.5 text-xs text-purple-200"
                        >
                            Important
                        </span>
                    {/if}
                    {#if hypaV3Data.metrics.lastRecentSummaries.includes(summaryIndex)}
                        <span class="whitespace-nowrap rounded-full bg-blue-900/70 px-1.5 py-0.5 text-xs text-blue-200">
                            Recent
                        </span>
                    {/if}
                    {#if hypaV3Data.metrics.lastSimilarSummaries.includes(summaryIndex)}
                        <span
                            class="whitespace-nowrap rounded-full bg-green-900/70 px-1.5 py-0.5 text-xs text-green-200"
                        >
                            Similar
                        </span>
                    {/if}
                    {#if hypaV3Data.metrics.lastRandomSummaries.includes(summaryIndex)}
                        <span
                            class="whitespace-nowrap rounded-full bg-yellow-900/70 px-1.5 py-0.5 text-xs text-yellow-200"
                        >
                            Random
                        </span>
                    {/if}
        ...
        <div class="flex items-center gap-2">
            ...
            >
                <ScissorsLineDashed class="h-4 w-4" />
            </button>
        </div>
    </div>
    ...
    <div class="mt-2 sm:mt-4">
        <textarea
            class="resize-vertical min-h-40 w-full rounded border border-zinc-700 bg-zinc-900 p-2 text-zinc-200 transition-colors focus:outline-none focus:ring-2 focus:ring-zinc-500 sm:min-h-56 sm:p-4"
    ...
    {#if translation}
        <div class="mt-2 sm:mt-4">
            <div class="mb-2 text-sm text-zinc-400 sm:mb-4">
                {language.hypaV3Modal.translationLabel}
            </div>

            <textarea
                class="resize-vertical min-h-40 w-full rounded border border-zinc-700 bg-zinc-900 p-2 text-zinc-200 transition-colors focus:outline-none sm:min-h-56 sm:p-4"
                readonly
                tabindex="-1"
    ...
    <div class="mt-2 flex flex-wrap gap-2 sm:mt-4">
        {#key summary.chatMemos.length}
            {#each summary.chatMemos as chatMemo, memoIndex (chatMemo)}
                <button
                    class="rounded-full bg-zinc-900 px-3 py-2 text-xs text-zinc-200 transition-colors hover:bg-zinc-700 {isMessageExpanded(
                        chatMemo
                    )
