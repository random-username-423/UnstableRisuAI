export const languageChineseTraditional = {
    ...
    errors: {
        ...
        networkFetchWeb:
            "這可能是由 CORS 錯誤引起的。這種情況只會在使用網頁版時發生，因為瀏覽器存在一些限制。請嘗試使用本地版或其他版本的叡甦。",
    ...
    help: {
        ...
        tempature:
            "較低的數值會使角色更緊密地遵循提示詞，但會使回應更制式與機械化。\n較高的數值則會增強角色的創意表現，但回應可能會變得不穩定。",
        ...
        loreRandomActivation:
            "啟用「使用機率條件」後，若同時符合啟用條目的其他條件，則在每次發送聊天時，該條目將依照設定的機率被使用。",
        additionalAssets:
            "在聊天中顯示的額外資源。\n\n - 使用 `{{raw::<資源名稱>}}` 作為路徑。\n - 使用 `{{image::<資源名稱>}}` 作為圖片。\n - 使用 `{{video::<資源名稱>}}` 作為影片。\n - 使用 `{{audio::<資源名稱>}}` 作為音訊。\n    - 建議放置在背景 HTML 中。",
        superMemory:
        ...
        additionalText:
            "只有當 AI 認為有必要時，才會將該段文本添加到角色描述中。您可以在此處放置較長的文本。使用雙換行進行內容分隔。",
        ...
        romanizer:
            "Romanizer 是一個將非羅馬字母轉換為羅馬字母的外掛程式，用於減少請求資料時的 Token。這可能會導致輸出結果與原始模型不同。如果已在聊天中使用羅馬字母，不建議啟用。",
        oaiRandomUser:
            "啟用後，請求中的使用者參數將被隨機 UUID 替代，並在重新整理時修改。這可以用來防止 AI 識別使用者。",
        ...
        lorePlus:
            "LoreBook+ 是一項實驗性功能。使用向量資料庫（VectorDB），而不僅是字串匹配。目的在於提供更好的機器人創建體驗和更好的匹配效能。",
        ...
        genTimes:
            "此設定支援模型上的重滾（reroll）回應數量。除第一則回應外，其他回應將作為快取使用，以降低成本。但若未多次重滾回應，可能增加成本。",
        ...
        antiClaudeOverload:
            "若 Claude 過載發生，叡甦會透過繼續相同的提示來阻止它，以減少過載的機率。此功能僅適用於流式回應（Streamed Responses），對非官方 API 端點可能無效。",
        triggerScript:
        ...
        autoContinueChat:
            "啟用後，當聊天不以標點符號結束時，系統將嘗試繼續對話。請勿在不使用標點符號的語言中啟用此功能。",
        combineTranslation:
        ...
        lowLevelAccess:
            "啟用後，將開放需要高計算能力的功能，並允許通過角色中的觸發式執行 AI 模型。除非確實需要這些功能，否則不要啟用此選項。",
        triggerLLMPrompt:
        ...
        namespace:
            "命名空間（Namespace）是模組的唯一標識符，用於防止模組衝突，並與預設和其他模組等進行互動。若不確定如何填寫，建議留空。",
        moduleIntergration:
        ...
        extractJson:
            '此欄位不為空時，將從回應中提取特定的 JSON 資料。例如：想從回應 `{"response": {"text": ["hello"]}}` 中提取 `response.text[0]`，可以填寫 `response.text.0`。',
        translatorNote:
            "可在此處為每個角色添加獨特的翻譯提示，但僅適用於使用 Ax. 模型進行翻譯。要啟用此功能，請在語言設定中包含 `{{slot::tnote}}`。此功能不適用群組聊天。",
        groupInnerFormat:
            "用於定義群組聊天中非發言者角色的格式。此欄位不為空時，將使用此格式替代預設格式。若 `Group Other Bot Role` 設定為 `assistant`，該格式也將應用於發言者。",
        ...
        summarizationPrompt:
            "用於摘要的提示詞。留空將使用預設提示詞。您也可以使用包含 {{slot}} 的 ChatML 格式來處理聊天數據。",
        translatorPrompt:
            "用於翻譯的提示詞。留空將使用默認提示。您還可以使用帶有 {{slot}} 的 ChatML 格式表示目標語言：用 {{slot::content}} 表示內容，用 {{slot::tnote}} 表示翻譯註釋。",
        translateBeforeHTMLFormatting:
            "啟用後，將在正規表達式和 HTML 格式化之前翻譯文本。這可能減少 Token 數，但可能破壞格式。",
        autoTranslateCachedOnly: "啟用自動翻譯選項時，將只自動翻譯使用者先前已翻譯過的訊息。",
        presetChain:
            "若欄位不為空，則每次使用者發送訊息時，系統會從此輸入中的預設列表中隨機變更並應用預設設定。預設列表應以逗號分隔，例如 `preset1,preset2`。",
        ...
        APIPool:
            "啟用後，系統將連接到 RisuAI 的 API 資源池。已啟用的使用者可共享免費、速率受限模型的 API 金鑰，從而利用其他使用者未充分使用的金鑰，增加對速率受限模型的請求次數。",
    },
    setup: {
    ...
    connectionOpenInfo:
        "多人聊天室已開啟，您可以將聊天室代碼分享給其他使用者。其他使用者可在 Playground > 加入多人聊天室 > 使用代碼加入。",
    ...
    pluginVersionWarn:
        "這是 {{plugin_version}} 版本的外掛程式，與當前 RisuAI 版本不相容。請更新外掛至 {{required_version}} 版本。",
