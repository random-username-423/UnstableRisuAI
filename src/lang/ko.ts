export const languageKorean = {
    ...
    "help": {
        ...
        "antiServerOverload": "활성화하면 서버 과부하 에러 발생 시 기본 재시도 횟수의 2배까지 자동으로 재시도합니다. Claude (Anthropic) 및 Gemini API에서 작동합니다.",
        "newChatSeparator": "활성화하면 예제 메시지와 실제 채팅 사이에 '[Start a new chat]' 구분자를 삽입합니다. 모델이 Few-shot 예제와 실제 대화를 구분하는 데 도움이 됩니다.",
        ...
        "systemContentReplacement": "모델이 시스템 프롬프트를 지원하지 않을 경우 시스템 프롬프트를 대체하는 프롬프트 형식입니다.",
        "systemRoleReplacement": "모델이 시스템 역할을 지원하지 않을 경우 시스템 역할을 대체하는 역할입니다.",
        "summarizationPrompt": "요약에 사용되는 프롬프트입니다. 비어 있으면 기본 프롬프트를 사용합니다. 채팅 데이터용 {{slot}}과 함께 ChatML 형식을 사용할 수도 있습니다.",
        "translatorPrompt": "번역에 사용되는 프롬프트입니다. 비어 있으면 기본 프롬프트를 사용합니다. 대상 언어용 {{slot}}, 콘텐츠용 {{slot::content}}, 번역자 노트용 {{slot::tnote}}와 함께 ChatML 형식을 사용할 수도 있습니다.",
        "translateBeforeHTMLFormatting": "활성화하면 정규식 스크립트와 HTML 포맷팅 전에 텍스트를 번역합니다. 토큰을 줄일 수 있지만 포맷팅이 깨질 수 있습니다.",
        "presetChain": "비어 있지 않으면 사용자가 메시지를 보낼 때마다 이 입력의 프리셋 목록에서 무작위로 프리셋이 변경되어 적용됩니다. 프리셋 목록은 쉼표로 구분합니다. 예: `preset1,preset2`.",
        "legacyMediaFindings": "활성화하면 추가 검색 알고리즘 없이 이전 방식으로 미디어 에셋을 찾습니다.",
        "comfyWorkflow": "Comfy UI의 API 워크플로우를 입력하세요. Comfy UI에서 'Workflow > Export (API)' 버튼을 눌러 API 워크플로우를 얻을 수 있습니다. 워크플로우 텍스트에 {{risu_prompt}}를 반드시 넣어야 합니다. {{risu_prompt}}는 Risu가 제공하는 프롬프트로 대체됩니다.",
        "claudeAutomaticCaching": "프롬프트 템플릿에 캐시 포인트가 없으면 가장 마지막 유저 메시지 3개에 자동으로 캐시 포인트를 생성합니다.",
        "claude1HourCaching": "캐시 TTL을 5분에서 1시간으로 연장합니다. 긴 대화에서 캐시 미스율을 줄일 수 있습니다.",
        "claudeCachingRetrival": "4분 30초마다 요청을 보내 캐시 TTL을 연장합니다 (최대 5회). 캐시 미스율을 줄일 수 있지만, 제대로 사용하지 않으면 비용이 증가할 수 있습니다.",
        "claudeBatching": "Claude 배치 API를 사용하여 요청을 처리합니다. 비용을 50% 절감할 수 있지만, 응답이 최대 24시간 걸릴 수 있습니다.",
        "experimentalChatCompressionDesc": "사용하지 않는 채팅 데이터를 압축하여 별도 파일에 저장합니다. 채팅 데이터 크기를 크게 줄이고 성능을 크게 향상시키지만, 실험적 기능이라 불안정할 수 있으며 백업 기능 등에 문제가 발생할 수 있습니다.",
        ...
        "dynamicMemory": "활성화하면 어시스턴트가 응답 시간에 메모리 노트를 작성합니다. 이 기능을 활용하려면 추가 프롬프팅이 필요합니다.",
        ...
        "promptTextInfoInsideChat": "프롬프트 정보 표시 시 메타데이터뿐만 아니라 실제 프롬프트 템플릿 텍스트도 채팅에 포함합니다.",
        "returnCSSError": "CSS 파싱 오류를 무시하지 않고 메시지로 표시합니다. 커스텀 CSS 디버깅에 유용합니다.",
        "newOAIHandle": "개선된 OpenAI 메시지 처리: 빈 메시지를 제거하고 예시 메시지 및 새 채팅 구분자를 올바르게 처리합니다.",
        "googleCloudTokenization": "Gemini 모델에 Google Cloud의 공식 토크나이저 API를 사용합니다. 로컬 추정보다 정확하지만 네트워크 요청이 필요합니다.",
        "loreBookDepth": "로어북 항목 매칭 시 검색할 최근 메시지 수입니다. 값이 높을수록 더 많은 로어북 항목이 활성화될 수 있습니다.",
        "loreBookToken": "로어북 콘텐츠에 할당할 최대 토큰 수입니다. 다른 콘텐츠를 위한 컨텍스트 공간을 확보하기 위해 로어 데이터 양을 제한합니다.",
        "assetMaxDifference": "캐릭터 에셋 퍼지 매칭 시 허용되는 최대 차이입니다. 값이 높을수록 느슨한 매칭을 허용합니다 (예: 'portrait01'이 'portrait1'과 매칭).",
        "newImageHandlingBeta": "채팅 메시지의 실험적 이미지 처리를 활성화합니다. 에셋 이미지 참조를 자동으로 해결하고 스타일을 적용합니다.",
        "noWaitForTranslate": "DeepLX 번역 요청의 속도 제한을 건너뜁니다. 비활성화 시 서버 과부하 방지를 위해 요청 간 3초 지연을 적용합니다.",
        "useExperimentalGoogleTranslator": "표준 API 대신 실험적 Google 번역 스크래퍼를 사용합니다. Tauri 또는 Node.js 서버 환경에서만 작동합니다.",
        "rememberToolUsage": "도구 호출 결과를 대화 기록에 저장하여 AI가 향후 응답에서 참조할 수 있도록 합니다.",
        "simplifiedToolUse": "도구 호출 시 AI의 텍스트 응답을 숨기고 도구 결과만 표시합니다. 비활성화 시 AI 추론과 도구 출력을 모두 포함합니다.",
        "autoContinueMinTokens": "응답의 최소 토큰 수입니다. 응답이 이보다 짧으면 자동으로 대화를 계속하여 목표 길이에 도달합니다 (0 = 비활성화).",
        "removeIncompleteResponse": "응답 끝의 불완전한 문장을 자동으로 제거합니다. 적절한 문장 부호가 나올 때까지 후행 텍스트를 제거합니다.",
        "additionalPrompt": "시스템 프롬프트에 추가되는 추가 지시사항입니다. 프롬프트 전처리가 활성화된 경우에만 사용됩니다.",
        "descriptionPrefix": "시스템 프롬프트에서 캐릭터 설명 앞에 추가되는 텍스트입니다. 프롬프트 전처리가 활성화된 경우에만 사용됩니다.",
        "useTokenizerCaching": "토큰화 결과를 캐시하여 반복적인 토큰 계산 속도를 향상시킵니다. CPU 사용량을 줄이지만 메모리를 더 사용합니다.",
        ...
        "heightMode": "뷰포트의 CSS 높이 단위입니다. 표준은 vh, 모바일 동적 높이는 dvh, 대형/소형 화면은 lvh/svh를 사용하세요.",
        "allowAllExtentionFiles": "파일 선택 대화 상자에서 확장자 제한을 무시하고 모든 파일 형식을 선택할 수 있도록 합니다.",
        ...
        "syncHelp": "실시간 동기화는 Google Drive와 데이터를 자동으로 동기화합니다. 숨겨진 appDataFolder를 사용합니다 (Drive 용량에 포함되지 않음). 변경 후 5초 대기 후 자동으로 동기화됩니다."
    },
    "setup": {
        ...
        // translated by Claude Code
        "igpPrompt": "IGP 프롬프트가 비어 있지 않으면 메인 모델 요청 후, 메인 모델 실행 후에 감정 모델로서 실행되어 결과를 메인 모델 응답 뒤에 추가합니다."
    },
    "triggerCategories": {
        "Special": "특수",
        "Control": "제어",
    ...
    "pluginVersionWarn": "이것은 플러그인의 {{plugin_version}} 버전입니다. 이 버전의 RisuAI와 호환되지 않습니다. 플러그인을 {{required_version}} 버전으로 업데이트해주세요.",
    ...
    "claudeCachingRetrivalDesc": "4분마다 요청하여 Claude 캐싱의 캐시 시간을 연장합니다. 캐시 미스율을 줄일 수 있지만, 제대로 사용하지 않으면 비용이 증가할 수 있습니다.",
