export const languageKorean = {
    ...
    help: {
        ...
        frequencyPenalty:
            "값이 높을수록 응답 내에서 대사가 반복되는 걸 줄여주지만, 값이 높으면 캐릭터의 반응이 이상해질 수 있습니다.",
        presensePenalty:
            "값이 높을수록 전체 콘텍스트 내에서 대사가 반복되는 걸 줄여주지만, 값이 높으면 캐릭터의 반응이 이상해질 수 있습니다.",
        ...
        scenario:
            "캐릭터의 시나리오에 대한 간단한 설명입니다. **이 옵션을 사용하는 것은 권장되지 않습니다. 대신 캐릭터 설명에 설명하십시오.**",
        utilityBot:
            "활성화되면 메인 프롬프트, jailbreak 및 기타 프롬프트를 무시합니다. 역할극이 아닌 유틸리티용 봇에 사용됩니다.",
        loreSelective: "선택 모드가 토글되면 활성화 키와 보조 키 모두 일치해야 로어가 활성화됩니다.",
        loreRandomActivation:
            "확률 조건 사용이 활성화된 경우, 로어의 다른 조건이 모두 충족되면 로어가 활성화되며, 각 채팅을 보낼 때마다 설정된 확률에 따라 활성화됩니다.",
        additionalAssets:
        ...
        oaiRandomUser:
            "활성화되면 요청에 사용자 매개 변수로 임의의 UUID가 입력되고 새로 고침할 때마다 변경됩니다. AI가 사용자를 식별하는 것을 방지하는 데 사용할 수 있습니다.",
        ...
        showUnrecommended:
            "활성화되면 권장되지 않는, 사용되지 않는 설정을 표시합니다. 이러한 설정을 사용하는 것을 권장하지 않습니다.",
        imageCompression:
            "활성화되면 캐릭터 내보내기 시 이미지를 압축합니다. 애니메이션 이미지가 작동하지 않는 경우 이 옵션을 비활성화해보세요.",
        useExperimental: "활성화되면 일부 실험적인 기능이 표시됩니다.",
        forceProxyAsOpenAI: "활성화되면 커스텀 모델 사용 시 OpenAI 형식을 강제로 사용합니다.",
        forcePlainFetch:
            "활성화되면 네이티브 HTTP 요청 대신 브라우저 Fetch API를 사용합니다. 이로 인해 CORS 오류가 발생할 수 있습니다.",
        autoFillRequestURL: "활성화되면 요청 URL을 현재 모델과 일치하도록 자동으로 채웁니다.",
        chainOfThought: "활성화되면 생각의 사슬 프롬프트를 프롬프트에 추가합니다.",
        gptVisionQuality:
            "OpenAI의 이미지 감지 모델 품질을 설정합니다. 품질이 높을수록 감지가 더 정확하지만 토큰이 더 많이 사용됩니다.",
        openAIServiceTier:
            "OpenAI API 서비스 티어. 'Flex'는 응답 시간이 느린 대신 50% 비용 절감. 최신 추론 모델에서 사용 가능.",
        geminiVisionQuality:
            "Gemini의 미디어 해상도를 설정합니다. 해상도가 높을수록 감지가 더 정확하지만 토큰이 더 많이 사용됩니다. Gemini 3 기준: Low=280, Medium=560, High=1120, Unspecified=1120 토큰/이미지. 다른 모델은 다를 수 있습니다.",
        geminiMergeSystemToUser:
            "활성화하면 Gemini의 system_instruction 필드 대신 system 메시지를 user 메시지에 병합합니다. 시스템 프롬프트에 문제가 있는 경우 이 옵션을 활성화하세요.",
        genTimes:
            "동일한 인풋으로 여러 개의 아웃풋을 생성합니다. 예: 3으로 설정 시 인풋 비용 x1, 아웃풋 비용 x3. (일반적으로 3번 요청하면 인풋 x3, 아웃풋 x3)",
        requestretrys: "이 옵션은 요청이 실패할 때 요청을 다시 시도할 횟수를 설정하는 데 사용됩니다.",
        antiServerOverload:
            "활성화하면 서버 과부하 에러 발생 시 기본 재시도 횟수의 2배까지 자동으로 재시도합니다. Claude (Anthropic) 및 Gemini API에서 작동합니다.",
        newChatSeparator:
            "활성화하면 예제 메시지와 실제 채팅 사이에 '[Start a new chat]' 구분자를 삽입합니다. 모델이 Few-shot 예제와 실제 대화를 구분하는 데 도움이 됩니다.",
        checkCorruption: "활성화하면 로컬 백업 저장 전에 서버로 백업 데이터를 전송하여 데이터 무결성을 확인합니다.",
        emotionPrompt:
            "이 옵션은 감정을 감지하는 데 사용되는 프롬프트를 설정하는 데 사용됩니다. 비어 있으면 기본 프롬프트를 사용합니다.",
        ...
        // translated by Claude Code
        systemContentReplacement:
            "모델이 시스템 프롬프트를 지원하지 않을 경우 시스템 프롬프트를 대체하는 프롬프트 형식입니다.",
        systemRoleReplacement: "모델이 시스템 역할을 지원하지 않을 경우 시스템 역할을 대체하는 역할입니다.",
        summarizationPrompt:
            "요약에 사용되는 프롬프트입니다. 비어 있으면 기본 프롬프트를 사용합니다. 채팅 데이터용 {{slot}}과 함께 ChatML 형식을 사용할 수도 있습니다.",
        translatorPrompt:
            "번역에 사용되는 프롬프트입니다. 비어 있으면 기본 프롬프트를 사용합니다. 대상 언어용 {{slot}}, 콘텐츠용 {{slot::content}}, 번역자 노트용 {{slot::tnote}}와 함께 ChatML 형식을 사용할 수도 있습니다.",
        translateBeforeHTMLFormatting:
            "활성화하면 정규식 스크립트와 HTML 포맷팅 전에 텍스트를 번역합니다. 토큰을 줄일 수 있지만 포맷팅이 깨질 수 있습니다.",
        presetChain:
            "비어 있지 않으면 사용자가 메시지를 보낼 때마다 이 입력의 프리셋 목록에서 무작위로 프리셋이 변경되어 적용됩니다. 프리셋 목록은 쉼표로 구분합니다. 예: `preset1,preset2`.",
        legacyMediaFindings: "활성화하면 추가 검색 알고리즘 없이 이전 방식으로 미디어 에셋을 찾습니다.",
        comfyWorkflow:
            "Comfy UI의 API 워크플로우를 입력하세요. Comfy UI에서 'Workflow > Export (API)' 버튼을 눌러 API 워크플로우를 얻을 수 있습니다. 워크플로우 텍스트에 {{risu_prompt}}를 반드시 넣어야 합니다. {{risu_prompt}}는 Risu가 제공하는 프롬프트로 대체됩니다.",
        claudeAutomaticCaching:
            "프롬프트 템플릿에 캐시 포인트가 없으면 가장 마지막 유저 메시지 3개에 자동으로 캐시 포인트를 생성합니다.",
        claude1HourCaching: "캐시 TTL을 5분에서 1시간으로 연장합니다. 긴 대화에서 캐시 미스율을 줄일 수 있습니다.",
        claudeCachingRetrival:
            "4분 30초마다 요청을 보내 캐시 TTL을 연장합니다 (최대 5회). 캐시 미스율을 줄일 수 있지만, 제대로 사용하지 않으면 비용이 증가할 수 있습니다.",
        claudeBatching:
            "Claude 배치 API를 사용하여 요청을 처리합니다. 비용을 50% 절감할 수 있지만, 응답이 최대 24시간 걸릴 수 있습니다.",
        experimentalChatCompressionDesc:
            "사용하지 않는 채팅 데이터를 압축하여 별도 파일에 저장합니다. 채팅 데이터 크기를 크게 줄이고 성능을 크게 향상시키지만, 실험적 기능이라 불안정할 수 있으며 백업 기능 등에 문제가 발생할 수 있습니다.",
        autoAdjustSchema: "활성화하면 Dynamic Output에 대한 JSON 스키마를 자동으로 조정합니다.",
        dynamicMessages: "활성화하면 어시스턴트가 한 번에 하나씩이 아닌 여러 메시지를 연속으로 보낼 수 있습니다.",
        dynamicMemory:
            "활성화하면 어시스턴트가 응답 시간에 메모리 노트를 작성합니다. 이 기능을 활용하려면 추가 프롬프팅이 필요합니다.",
        dynamicResponseTiming: "활성화하면 응답 타이밍을 동적으로 조정합니다.",
        ...
        pastThinkingSend:
            "AI 모델(Claude, Gemini 등)의 이전 추론 과정을 다음 요청에 포함하는 방식을 설정합니다.\n\n- **포함 안 함**: 이전 추론을 전송하지 않습니다.\n- **포함**: 이전 추론을 컨텍스트에 포함합니다.\n- **포함 (추가 컨텍스트)**: 이전 추론에 별도 토큰 예산을 사용합니다. 추론만 의도적으로 적게 유지하는 것이 가능하지만, 경우에 따라 캐시가 쉽게 무효화될 수 있습니다.\n\n`<Thoughts>` 태그의 내용은 이 설정과 관련 없이 전송되지 않습니다.",
        enableDevTools: "사이드바에 개발자 도구 버튼을 표시합니다. 디버깅 유틸리티와 캐릭터 데이터 검사에 사용됩니다.",
        promptTextInfoInsideChat:
            "프롬프트 정보 표시 시 메타데이터뿐만 아니라 실제 프롬프트 템플릿 텍스트도 채팅에 포함합니다.",
        returnCSSError: "CSS 파싱 오류를 무시하지 않고 메시지로 표시합니다. 커스텀 CSS 디버깅에 유용합니다.",
        newOAIHandle:
            "개선된 OpenAI 메시지 처리: 빈 메시지를 제거하고 예시 메시지 및 새 채팅 구분자를 올바르게 처리합니다.",
        googleCloudTokenization:
            "Gemini 모델에 Google Cloud의 공식 토크나이저 API를 사용합니다. 로컬 추정보다 정확하지만 네트워크 요청이 필요합니다.",
        loreBookDepth:
            "로어북 항목 매칭 시 검색할 최근 메시지 수입니다. 값이 높을수록 더 많은 로어북 항목이 활성화될 수 있습니다.",
        loreBookToken:
            "로어북 콘텐츠에 할당할 최대 토큰 수입니다. 다른 콘텐츠를 위한 컨텍스트 공간을 확보하기 위해 로어 데이터 양을 제한합니다.",
        assetMaxDifference:
            "캐릭터 에셋 퍼지 매칭 시 허용되는 최대 차이입니다. 값이 높을수록 느슨한 매칭을 허용합니다 (예: 'portrait01'이 'portrait1'과 매칭).",
        newImageHandlingBeta:
            "채팅 메시지의 실험적 이미지 처리를 활성화합니다. 에셋 이미지 참조를 자동으로 해결하고 스타일을 적용합니다.",
        noWaitForTranslate:
            "DeepLX 번역 요청의 속도 제한을 건너뜁니다. 비활성화 시 서버 과부하 방지를 위해 요청 간 3초 지연을 적용합니다.",
        useExperimentalGoogleTranslator:
            "표준 API 대신 실험적 Google 번역 스크래퍼를 사용합니다. Tauri 또는 Node.js 서버 환경에서만 작동합니다.",
        rememberToolUsage: "도구 호출 결과를 대화 기록에 저장하여 AI가 향후 응답에서 참조할 수 있도록 합니다.",
        simplifiedToolUse:
            "도구 호출 시 AI의 텍스트 응답을 숨기고 도구 결과만 표시합니다. 비활성화 시 AI 추론과 도구 출력을 모두 포함합니다.",
        autoContinueMinTokens:
            "응답의 최소 토큰 수입니다. 응답이 이보다 짧으면 자동으로 대화를 계속하여 목표 길이에 도달합니다 (0 = 비활성화).",
        removeIncompleteResponse:
            "응답 끝의 불완전한 문장을 자동으로 제거합니다. 적절한 문장 부호가 나올 때까지 후행 텍스트를 제거합니다.",
        additionalPrompt:
            "시스템 프롬프트에 추가되는 추가 지시사항입니다. 프롬프트 전처리가 활성화된 경우에만 사용됩니다.",
        descriptionPrefix:
            "시스템 프롬프트에서 캐릭터 설명 앞에 추가되는 텍스트입니다. 프롬프트 전처리가 활성화된 경우에만 사용됩니다.",
        useTokenizerCaching:
            "토큰화 결과를 캐시하여 반복적인 토큰 계산 속도를 향상시킵니다. CPU 사용량을 줄이지만 메모리를 더 사용합니다.",
        requestLocation: "API 요청에 전송되는 위치 헤더입니다. 옵션: 기본값, EU (GDPR 준수), US (FedRAMP).",
        keiServerURL: "KEI 클라우드 백업용 커스텀 서버 URL입니다. 기본 Risu 서버를 사용하려면 비워두세요.",
        heightMode:
            "뷰포트의 CSS 높이 단위입니다. 표준은 vh, 모바일 동적 높이는 dvh, 대형/소형 화면은 lvh/svh를 사용하세요.",
        allowAllExtentionFiles:
            "파일 선택 대화 상자에서 확장자 제한을 무시하고 모든 파일 형식을 선택할 수 있도록 합니다.",
        personaNote: "각 페르소나에 메모 필드를 표시합니다. 다른 페르소나 버전을 구분하는 데 유용합니다.",
        showDeprecatedTriggerV1:
            "더 이상 사용되지 않는 V1 트리거 형식 버튼을 표시합니다. 레거시 V1 트리거를 사용하는 경우에만 활성화하세요.",
        lightningRealmImport:
            "Realm에서 캐릭터 가져오기 시 에셋을 클라우드 스토리지에 업로드합니다. Risu 계정 동기화가 필요합니다.",
        banCharacterset:
            "지정된 유니코드 스크립트(예: 한자, 한글)를 포함하는 AI 응답을 거부합니다. 응답에서 특정 언어를 강제하는 데 유용합니다.",
        driveParallelConnections:
            "Google Drive 백업/복원 시 동시 연결 수입니다. 높을수록 전송 속도가 빨라지지만 속도 제한이 발생할 수 있습니다. 기본값: 20.",
        maxDbBackups:
            "디바이스 내부에 저장되는 DB 백업의 최대 개수입니다. 충돌 복구용 숨겨진 백업이며, DB만 저장됩니다 (에셋 제외). 각 백업은 DB 크기와 동일합니다. 낮출수록 저장 공간이 절약됩니다. 기본값: 20.",
        dbBackupIntervalMinutes:
            "내부 DB 백업 사이의 최소 시간(분)입니다. 마지막 백업 이후 이 시간이 지나지 않으면 새 백업 생성을 건너뜁니다. 값이 클수록 저장 쓰기가 줄지만 충돌 시 더 많은 데이터가 손실될 수 있습니다. 기본값: 10분.",
        syncHelp:
            "실시간 동기화는 Google Drive와 데이터를 자동으로 동기화합니다. 숨겨진 appDataFolder를 사용합니다 (Drive 용량에 포함되지 않음). 변경 후 5초 대기 후 자동으로 동기화됩니다.",
    },
    setup: {
        ...
        // translated by Claude Code
        igpPrompt:
            "IGP 프롬프트가 비어 있지 않으면 메인 모델 요청 후, 메인 모델 실행 후에 감정 모델로서 실행되어 결과를 메인 모델 응답 뒤에 추가합니다.",
    },
    triggerCategories: {
        Special: "특수",
        Control: "제어",
    ...
    // 동기화
    syncTitle: "실시간 동기화",
    syncEnabled: "실시간 동기화 활성화",
    syncStatus: "동기화 상태",
    syncStatusIdle: "대기 중",
    syncStatusSyncing: "동기화 중...",
    syncStatusError: "오류",
    syncStatusConflict: "충돌 감지됨",
    syncLastSync: "마지막 동기화",
    syncNever: "없음",
    syncUpload: "서버에 업로드",
    syncDownload: "서버에서 다운로드",
    syncUploadConfirm: "로컬 데이터를 Google Drive에 업로드하시겠습니까? 서버 데이터가 덮어씌워집니다.",
    syncDownloadConfirm: "Google Drive에서 데이터를 다운로드하시겠습니까? 로컬 데이터가 덮어씌워집니다.",
    syncConnectDrive: "Google Drive 연결",
    syncDisconnect: "연결 해제",
    syncNoToken: "Google Drive에 연결되지 않음",
    syncConflictDetected: "동기화 충돌이 감지되었습니다! 다른 기기에서 변경사항이 있습니다.",
    syncUseServerVersion: "서버 버전 사용 (다운로드)",
    syncUseLocalVersion: "내 버전 사용 (업로드)",
    syncHelp: "실시간 동기화는 Google Drive를 통해 여러 기기 간에 데이터를 동기화합니다.",
    syncStart: "실시간 동기화 시작",
    syncStop: "동기화 중지",
    syncConnected: "연결 완료! 동기화가 시작되었습니다.",
    syncDisconnected: "동기화가 중지되었습니다.",
    syncUploadingAssets: "에셋 업로드 중...",
    syncDownloadingAssets: "에셋 다운로드 중...",
    files: "파일",
    ...
    confirmRecommendedPreset:
        "이 모델에 대한 추천 설정이 있습니다. 그걸로 변경하시겠습니까? (접근성 설정에서 물어보는 것을 끌 수 있습니다.)",
    ...
    advSettingsClaudeSection: "Claude 설정",
    advSettingsOpenAISection: "OpenAI 설정",
    advSettingsGoogleSection: "Google 설정",
    advSettingsLoreBookSection: "Lore Book 설정",
    advSettingsImageMediaSection: "이미지/미디어 설정",
    advSettingsTranslationSection: "번역 설정",
    advSettingsToolSection: "Tool 설정",
    advSettingsChatResponseSection: "채팅/응답 설정",
    advSettingsPromptSection: "프롬프트 설정",
    advSettingsPerformanceSection: "성능 설정",
    advSettingsNetworkSection: "네트워크 설정",
    advSettingsDevDebugSection: "개발자/디버그 설정",
    advSettingsChatInputSection: "채팅 입력 설정",
    advSettingsOtherSection: "기타 설정",

    // translated by Claude Code
    notification: "알림",
    permissionDenied: "브라우저 또는 OS에서 권한이 거부되었습니다",
    customFlags: "커스텀 플래그",
    enableCustomFlags: "커스텀 플래그 활성화",
    googleCloudTokenization: "Google Cloud 토큰화",
    geminiMergeSystemToUser: "System을 User에 병합 (레거시)",
    presetChain: "프리셋 체인",
    legacyMediaFindings: "레거시 미디어 검색",
    staticsDisclaimer: "통계는 2024년 7월 이후의 데이터를 기반으로 합니다. 데이터가 정확하지 않을 수 있습니다.",
    subtitles: "자막",
    ...
    destinationLanguage: "대상 언어",
    noWebGPU: "브라우저 또는 OS가 WebGPU를 지원하지 않습니다. 성능이 크게 저하될 수 있습니다.",
    menuSideBar: "메뉴 사이드바",
    home: "홈",
    showSavingIcon: "저장 아이콘 표시",
    pluginVersionWarn:
        "이것은 플러그인의 {{plugin_version}} 버전입니다. 이 버전의 RisuAI와 호환되지 않습니다. 플러그인을 {{required_version}} 버전으로 업데이트해주세요.",
    imageTranslation: "이미지 번역",
    banCharacterset: "문자셋 자동 재생성",
    checkCorruption: "백업 데이터 손상 확인",
    backupTooLargeError:
        "백업 데이터가 너무 커서 검증할 수 없습니다. 고급 설정 > 네트워크 섹션에서 '백업 데이터 손상 확인'을 끄고 다시 시도해주세요.",
    ...
    claudeCachingRetrival: "Claude 캐싱 유지",
    claudeCachingRetrivalDesc:
        "4분마다 요청하여 Claude 캐싱의 캐시 시간을 연장합니다. 캐시 미스율을 줄일 수 있지만, 제대로 사용하지 않으면 비용이 증가할 수 있습니다.",
    claudeAutomaticCaching: "Claude 자동 캐싱",
    ...
    seperateModelsForAxModels: "보조 모델용 별도 모델",
    axModelsDef: "보조 모델 정의",
    doNotChangeSeperateModels: "프리셋 변경 시 별도 모델 변경하지 않음",
    tools: "도구",
    action: "액션",
    hotkey: "단축키",
    hotkeyDesc: {
    ...
    fallbackModel: "폴백 모델",
    fallbackWhenBlankResponse: "빈 응답 시 폴백",
    doNotChangeFallbackModels: "프리셋 변경 시 폴백 모델 변경하지 않음",
    customModels: "커스텀 모델",
    igpPrompt: "IGP 프롬프트",
    useTokenizerCaching: "토크나이저 캐싱",
    claude1HourCaching: "Claude 1시간 캐싱",
    personaNote: "페르소나 노트",
    mcpAccessPrompt: '{{tool}}이(가) "{{action}}"을(를) 시도합니다. 허용하시겠습니까?',
    rememberToolUsage: "도구 사용 기억",
    simplifiedToolUse: "간소화된 도구 사용",
    toolCalled: "'{{tool}}' 도구 호출됨",
    newImageHandlingBeta: "새 이미지 처리 (베타)",
    settingsExported: "버그 리포트용 설정이 내보내지고 클립보드에 복사되었습니다.",
    dynamicOutput: "동적 출력",
    autoAdjustSchema: "자동 스키마 조정",
