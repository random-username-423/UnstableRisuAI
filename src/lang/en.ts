export const languageEnglish = {
    ...
    help:{
        ...
        driveParallelConnections: "Number of parallel connections for Google Drive backup/restore. Higher values can speed up transfers but may cause rate limiting. Default: 20.",
        maxDbBackups: "Maximum number of internal DB backups stored on your device. These are hidden backups used for crash recovery (DB only, not assets). Each backup equals your DB size. Lower values save storage. Default: 20.",
        dbBackupIntervalMinutes: "Minimum time between internal DB backups in minutes. If less time has passed since the last backup, new backup creation is skipped. Higher values reduce storage writes but may lose more data on crash. Default: 10 minutes.",
        antiServerOverload: "When enabled, automatically retries with double the retry count when server overload errors occur. Works with Claude (Anthropic) and Gemini APIs.",
        newChatSeparator: "When enabled, inserts '[Start a new chat]' separator between example messages and actual chat. This helps the model distinguish few-shot examples from real conversation.",
        checkCorruption: "When enabled, sends backup data to the server to verify data integrity before saving local backups.",
        ...
        claudeAutomaticCaching: "If there is no cache point in the prompt template, automatically creates cache points on the last 3 user messages.",
        claude1HourCaching: "Extends the cache TTL from 5 minutes to 1 hour. This can reduce cache miss rate for longer conversations.",
        claudeCachingRetrival: "Sends a request every 4.5 minutes (up to 5 times) to extend the cache TTL. This can reduce cache miss rate, but may increase cost if not used properly.",
        claudeBatching: "Uses Claude's batch API to process requests. This can reduce cost by 50%, but responses may take up to 24 hours.",
        ...
        enableDevTools: "Show the Developer Tools button in the sidebar for accessing debugging utilities and inspecting character data.",
        promptTextInfoInsideChat: "Include the actual prompt template text (not just metadata) in the chat messages when displaying prompt information.",
        returnCSSError: "Display CSS parsing errors as visible messages instead of silently ignoring them. Useful for debugging custom CSS.",
        newOAIHandle: "Improved OpenAI message handling: removes empty messages and properly handles example messages and new chat separators.",
        googleCloudTokenization: "Use Google Cloud's official tokenization API for Gemini models instead of local estimation. More accurate but requires network request.",
        loreBookDepth: "Number of recent messages to search when finding matching lorebook entries. Higher values may activate more lorebook entries.",
        loreBookToken: "Maximum tokens allocated for lorebook content. Limits how much lore data is included to preserve context for other content.",
        assetMaxDifference: "Maximum allowed difference when fuzzy matching character assets. Higher values allow more loose matching (e.g., 'portrait01' matching 'portrait1').",
        newImageHandlingBeta: "Enable experimental image handling in chat messages. Automatically resolves asset image references and applies styling.",
        noWaitForTranslate: "Skip rate limiting for DeepLX translation requests. When disabled, enforces 3-second delays between requests to prevent server overload.",
        useExperimentalGoogleTranslator: "Use experimental Google Translate scraper instead of standard API. Only works in Tauri or Node.js server environments.",
        rememberToolUsage: "Save tool call results in conversation history so the AI can reference them in future responses.",
        simplifiedToolUse: "Hide AI's text response when tools are called, showing only tool results. When disabled, includes both AI reasoning and tool outputs.",
        autoContinueMinTokens: "Minimum token count for responses. If response is shorter, automatically continue the conversation to reach this target length (0 = disabled).",
        removeIncompleteResponse: "Automatically trim incomplete sentences from response end. Removes trailing text until a proper punctuation mark is found.",
        additionalPrompt: "Extra instructions appended to the system prompt. Only used when Prompt Preprocess is enabled.",
        descriptionPrefix: "Text prepended before the character's description in system prompts. Only used when Prompt Preprocess is enabled.",
        useTokenizerCaching: "Cache tokenization results to speed up repeated token counting. Reduces CPU usage but uses more memory.",
        requestLocation: "Location header sent with API requests for server-side routing. Options: Default, EU (GDPR compliant), US (FedRAMP).",
        keiServerURL: "Custom backup server URL for KEI cloud backups. Leave empty to use default Risu server.",
        heightMode: "CSS height unit for viewport. Use vh for standard, dvh for dynamic height (mobile), lvh/svh for large/small screens.",
        ...
        lightningRealmImport: "When importing characters from Realm, upload assets to cloud storage. Requires Risu account sync.",
        banCharacterset: "Reject AI responses containing specified Unicode scripts (e.g., Han, Hangul). Useful to enforce specific languages in responses.",
        syncHelp: "Real-time sync keeps your data synchronized with Google Drive. Uses the hidden appDataFolder (doesn't count against your Drive quota). Changes are automatically synced after a 5-second debounce delay."
    },
    setup: {
