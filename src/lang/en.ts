export const languageEnglish = {
    ...
    help: {
        ...
        submodel:
            "Auxiliary Model is a model that used in analizing emotion images and auto suggestions and etc. gpt3.5 is recommended.",
        ...
        chatNote:
            "a note that strongly effects model behavior. embbedded to current chat. also known as memory or ujb.",
        personality:
        ...
        additionalText:
            "The text that would be added to Character Description only when ai thinks its needed, so you can put long texts here. seperate with double newlines.",
        ...
        oaiRandomUser:
            "If enabled, random uuid would be put on user parameter on request, and would be changed on refresh. this can be used to prevent AI from identifying user.",
        ...
        gptVisionQuality:
            "This option is used to set the quality of the image detection model for OpenAI. The higher the quality, the more accurate the detection, but more tokens are used.",
        openAIServiceTier:
            "Service tier for OpenAI API. 'Flex' offers 50% cost reduction with slower response times. Available for latest reasoning models.",
        geminiVisionQuality:
            "This option is used to set the media resolution for Gemini. The higher the resolution, the more accurate the detection, but more tokens are used. For Gemini 3: Low=280, Medium=560, High=1120, Unspecified=1120 tokens per image. Other models may vary.",
        geminiMergeSystemToUser:
            "When enabled, system messages will be merged into user messages instead of using Gemini's native system_instruction field. Enable this if you experience issues with system prompts.",
        genTimes:
            "Generate multiple outputs from a single input. Example: Set to 3 = input cost x1, output cost x3. (Normally 3 requests = input x3, output x3)",
        requestretrys: "This option is used to set the number of request retrys when request fails.",
        driveParallelConnections:
            "Number of parallel connections for Google Drive backup/restore. Higher values can speed up transfers but may cause rate limiting. Default: 20.",
        maxDbBackups:
            "Maximum number of internal DB backups stored on your device. These are hidden backups used for crash recovery (DB only, not assets). Each backup equals your DB size. Lower values save storage. Default: 20.",
        dbBackupIntervalMinutes:
            "Minimum time between internal DB backups in minutes. If less time has passed since the last backup, new backup creation is skipped. Higher values reduce storage writes but may lose more data on crash. Default: 10 minutes.",
        antiServerOverload:
            "When enabled, automatically retries with double the retry count when server overload errors occur. Works with Claude (Anthropic) and Gemini APIs.",
        newChatSeparator:
            "When enabled, inserts '[Start a new chat]' separator between example messages and actual chat. This helps the model distinguish few-shot examples from real conversation.",
        checkCorruption:
            "When enabled, sends backup data to the server to verify data integrity before saving local backups.",
        emotionPrompt:
        ...
        autoContinueChat:
            "If enabled, it will try to continue the chat if it doesn't ends with a punctuation. DONT USE THIS WITH LANGUAGES THAT DOESN'T USE PUNCTUATION.",
        combineTranslation:
        ...
        claudeAutomaticCaching:
            "If there is no cache point in the prompt template, automatically creates cache points on the last 3 user messages.",
        claude1HourCaching:
            "Extends the cache TTL from 5 minutes to 1 hour. This can reduce cache miss rate for longer conversations.",
        claudeCachingRetrival:
            "Sends a request every 4.5 minutes (up to 5 times) to extend the cache TTL. This can reduce cache miss rate, but may increase cost if not used properly.",
        claudeBatching:
            "Uses Claude's batch API to process requests. This can reduce cost by 50%, but responses may take up to 24 hours.",
        ...
        pastThinkingSend:
            "Controls how previous thinking/reasoning from AI models (Claude, Gemini, etc.) is included in subsequent requests.\n\n- **None**: Previous thinking is not sent.\n- **Send**: Previous thinking is included in context.\n- **Send (Extra Context)**: Previous thinking uses a separate token budget. This allows you to intentionally keep reasoning minimal, but cache may be invalidated more easily in some cases.\n\nContent inside `<Thoughts>` tags is never sent regardless of this setting.",
        enableDevTools:
            "Show the Developer Tools button in the sidebar for accessing debugging utilities and inspecting character data.",
        promptTextInfoInsideChat:
            "Include the actual prompt template text (not just metadata) in the chat messages when displaying prompt information.",
        returnCSSError:
            "Display CSS parsing errors as visible messages instead of silently ignoring them. Useful for debugging custom CSS.",
        newOAIHandle:
            "Improved OpenAI message handling: removes empty messages and properly handles example messages and new chat separators.",
        googleCloudTokenization:
            "Use Google Cloud's official tokenization API for Gemini models instead of local estimation. More accurate but requires network request.",
        loreBookDepth:
            "Number of recent messages to search when finding matching lorebook entries. Higher values may activate more lorebook entries.",
        loreBookToken:
            "Maximum tokens allocated for lorebook content. Limits how much lore data is included to preserve context for other content.",
        assetMaxDifference:
            "Maximum allowed difference when fuzzy matching character assets. Higher values allow more loose matching (e.g., 'portrait01' matching 'portrait1').",
        newImageHandlingBeta:
            "Enable experimental image handling in chat messages. Automatically resolves asset image references and applies styling.",
        noWaitForTranslate:
            "Skip rate limiting for DeepLX translation requests. When disabled, enforces 3-second delays between requests to prevent server overload.",
        useExperimentalGoogleTranslator:
            "Use experimental Google Translate scraper instead of standard API. Only works in Tauri or Node.js server environments.",
        rememberToolUsage:
            "Save tool call results in conversation history so the AI can reference them in future responses.",
        simplifiedToolUse:
            "Hide AI's text response when tools are called, showing only tool results. When disabled, includes both AI reasoning and tool outputs.",
        autoContinueMinTokens:
            "Minimum token count for responses. If response is shorter, automatically continue the conversation to reach this target length (0 = disabled).",
        removeIncompleteResponse:
            "Automatically trim incomplete sentences from response end. Removes trailing text until a proper punctuation mark is found.",
        additionalPrompt:
            "Extra instructions appended to the system prompt. Only used when Prompt Preprocess is enabled.",
        descriptionPrefix:
            "Text prepended before the character's description in system prompts. Only used when Prompt Preprocess is enabled.",
        useTokenizerCaching:
            "Cache tokenization results to speed up repeated token counting. Reduces CPU usage but uses more memory.",
        requestLocation:
            "Location header sent with API requests for server-side routing. Options: Default, EU (GDPR compliant), US (FedRAMP).",
        keiServerURL: "Custom backup server URL for KEI cloud backups. Leave empty to use default Risu server.",
        heightMode:
            "CSS height unit for viewport. Use vh for standard, dvh for dynamic height (mobile), lvh/svh for large/small screens.",
        allowAllExtentionFiles: "Allow selecting any file type in file dialogs, ignoring extension restrictions.",
        personaNote: "Show a note field for each persona. Useful for identifying different persona versions.",
        showDeprecatedTriggerV1: "Show deprecated V1 trigger format button. Enable only if you use legacy V1 triggers.",
        lightningRealmImport:
            "When importing characters from Realm, upload assets to cloud storage. Requires Risu account sync.",
        banCharacterset:
            "Reject AI responses containing specified Unicode scripts (e.g., Han, Hangul). Useful to enforce specific languages in responses.",
        syncHelp:
            "Real-time sync keeps your data synchronized with Google Drive. Uses the hidden appDataFolder (doesn't count against your Drive quota). Changes are automatically synced after a 5-second debounce delay.",
    },
    setup: {
        ...
        claudeDesc:
            "Claude is a AI model made by Antropic, competitor to OpenAI. its better than GPT if you want to use it in other languages than English.",
        openAIDesc:
    ...
    syncEnabled: "Enable Real-time Sync",
    syncStatus: "Sync Status",
    syncStatusIdle: "Idle",
    syncStatusSyncing: "Syncing...",
    syncStatusError: "Error",
    syncStatusConflict: "Conflict Detected",
    syncLastSync: "Last Sync",
    syncNever: "Never",
    syncUpload: "Upload to Server",
    syncDownload: "Download from Server",
    syncUploadConfirm: "Upload local data to Google Drive? Server data will be overwritten.",
    syncDownloadConfirm: "Download data from Google Drive? Local data will be overwritten.",
    syncConnectDrive: "Connect Google Drive",
    syncDisconnect: "Disconnect",
    syncNoToken: "Not connected to Google Drive",
    syncConflictDetected: "Sync conflict detected! Another device has made changes.",
    syncUseServerVersion: "Use server version (download)",
    syncUseLocalVersion: "Use local version (upload)",
    syncHelp: "Real-time sync keeps your data synchronized across devices via Google Drive.",
    syncStart: "Start Real-time Sync",
    syncStop: "Stop Sync",
    syncConnected: "Connected! Sync started.",
    syncDisconnected: "Sync stopped.",
    syncUploadingAssets: "Uploading assets...",
    syncDownloadingAssets: "Downloading assets...",
    files: "Files",
    ...
    driveParallelConnections: "Google Drive Parallel Connections",
    maxDbBackups: "Internal DB Backup Limit",
    dbBackupIntervalMinutes: "Internal DB Backup Interval (minutes)",
    ...
    confirmRecommendedPreset:
        "There is a recommended settings for this model. do you want to change the settings to that? (you can turn off asking in the accessiblity settings)",
    ...
    connectionOpenInfo:
        "Multiuser room is opened. you can share the room code to other users. others can join the room by using the code inside playground > Join Multiuser Room.",
    ...
    storageFiles: "File Statistics",
    storageItems: "items",
    storageCharacters: "Characters",
    storageChats: "Chats",
    storageAssets: "Assets",
    storageBackups: "Backups",
    storageDatabaseFiles: "Database Files",
    storageFilesCount: "files",
    storageIndividual: "(individual files)",
    ...
    geminiMergeSystemToUser: "Merge System to User (Legacy)",
    ...
    backupTooLargeError:
        "Backup data is too large to verify. Please disable 'Check Backup Data Corruption' in Advanced Settings > Network section and try again.",
    ...
    claudeAutomaticCaching: "Claude Automatic Caching",
    ...
    advSettingsPerformanceSection: "Performance Settings",
    advSettingsNetworkSection: "Network Settings",
    advSettingsDevDebugSection: "Developer/Debug Settings",
    advSettingsChatInputSection: "Chat Input Settings",
