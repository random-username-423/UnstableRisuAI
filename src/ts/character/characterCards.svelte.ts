...
import { downloadFile, loadAsset, readImage, saveAsset } from "../utils/fileIO"
...
export const hubURL =
    typeof window !== "undefined" && (window as any).__NODE__ === true
        ? "/hub-proxy"
...
export const getRealmInfo = async (realmPath: string) => {
    ...
    showRealmInfoStore.value = await res.json()
}

export const showRealmInfoStore: { value: null | hubType } = $state({
    value: null,
})
    ...
    if (risuext && card.spec === "chara_card_v2") {
        ...
        if (risuext.vits) {
            ...
            for (let i = 0; i < keys.length; i++) {
                ...
                const imgp = await saveAsset(
                    mode === "hub" ? await getHubResources(risuext.vits[key]) : Buffer.from(risuext.vits[key], "base64")
                )
                risuext.vits[key] = imgp
            }
