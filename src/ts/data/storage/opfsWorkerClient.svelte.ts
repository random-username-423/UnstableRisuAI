...
const pendingListsWithSizes = new Map<
    string,
    { resolve: (files: { name: string; size: number }[]) => void; reject: (e: Error) => void }
>()
...
const pendingListsWithSizesRecursive = new Map<
    string,
    { resolve: (files: { path: string; size: number }[]) => void; reject: (e: Error) => void }
>()
const pendingDeleteDirectories = new Map<string, { resolve: () => void; reject: (e: Error) => void }>()
const pendingListEntries = new Map<
    string,
    { resolve: (entries: { name: string; size: number; isDirectory: boolean }[]) => void; reject: (e: Error) => void }
>()
 ...
 * OPFS 탐색기용
 */
export async function listEntriesFromWorker(
    dirPath: string
