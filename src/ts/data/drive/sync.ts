...
async function fetchWithRetry(url: string, options: FetchWithRetryOptions = {}): Promise<Response> {
    ...
    for (let attempt = 0; attempt <= maxRetries; attempt++) {
        ...
        if (response.status === 429 || response.status >= 500) {
            ...
            console.log(
                `[Sync] Rate limited (${response.status}), retrying in ${delaySeconds}s (attempt ${attempt + 1}/${maxRetries})`
            )
...
export async function showConflictDialog(
    ...
    const choice = await alertSelect([serverLabel, localLabel, language.cancel || "Cancel"])
 ...
 */
export async function performSync(accessToken: string, direction: "download" | "upload"): Promise<void> {
    const db = getDatabase()
    const serverManifest = await downloadManifest(accessToken)
...
async function syncDownload(
    ...
    db.characters = db.characters.filter((c) => serverManifest.characters[c.chaId] !== undefined)
    ...
    console.log(
        `[Sync] Download complete: ${downloadCount} downloaded, ${skipCount} skipped, ${assetResult.downloadCount} assets`
    )
}
...
async function syncUpload(
    ...
    console.log(
        `[Sync] Items to upload: ${charsToUpload.length} characters, ${chatsToUpload.length} chats, ${localAssets.length} assets, parallel: ${PARALLEL_UPLOADS}`
    )
    ...
    const settingsFileId = await uploadSyncFile(
        accessToken,
        "settings.json.bin",
        settingsContent,
        manifest.settings.fileId
    )
    ...
    const presetsFileId = await uploadSyncFile(
        accessToken,
        "botpresets.json.bin",
        presetsContent,
        manifest.botPresets.fileId
    )
    ...
    console.log(
        `[Sync] Upload complete: ${totalItems} items (${assetResult.uploadCount} assets uploaded, ${assetResult.skipCount} skipped)`
    )
}
...
export async function syncChangedItems(
    ...
    for (const item of changedItems) {
        if (item.type === "character" && item.id) {
            ...
            const data = await loadFromWorker(`database/chats/${chaId}/${chatId}.bin`)
            if (data) {
                const fileId = await uploadSyncFile(
                    accessToken,
                    fileName,
            ...
            const fileId = await uploadSyncFile(
                ...
                "settings.json.bin",
                settingsContent,
                serverManifest.settings.fileId
            )
            ...
            const fileId = await uploadSyncFile(
                ...
                "botpresets.json.bin",
                presetsContent,
                serverManifest.botPresets.fileId
            )
...
async function syncAssetsDownload(
    ...
    console.log(
        `[Sync Assets] Assets to download: ${toDownload.length}, skipping: ${skipCount}, parallel: ${PARALLEL_DOWNLOADS}`
    )
    ...
    if (toDownload.length > 0) {
        await Promise.all(Array.from({ length: Math.min(PARALLEL_DOWNLOADS, toDownload.length) }, () => downloadOne()))
    }
