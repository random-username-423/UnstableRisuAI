import { setDatabase, getDatabase } from "src/ts/data/storage/database.svelte"
import type { botPreset } from "src/ts/data/storage/types"
import { setPreset } from "src/ts/data/storage/utils/presetManager"
import {
    defaultAutoSuggestPrefixOoba,
    defaultAutoSuggestPrompt,
    defaultAutoSuggestPromptOoba,
} from "src/ts/data/storage/defaultPrompts"
...
export async function setRecommended(model: string, ask: "ask" | "force") {
    ...
    db.aiModel = model
    if (
        db.aiModel.startsWith("gpt") ||
        db.aiModel === "openrouter" ||
        db.aiModel === "reverse_proxy" ||
        db.aiModel.startsWith("claude")
    ) {
        ...
        const sel = parseInt(
            await alertSelect([
                "Vicuna, WizardLM, Airoboros",
                "OpenChat V3.2",
                "Guanaco",
                "OpenAssistant",
                "Dolphin, Luna",
                "StableBeluga, Orca-Mini",
                "Others (Alpaca, Nous-Hermes, ...)",
            ])
        )
