...
export async function requestOpenAIResponseAPI(arg: RequestDataArgumentExtended): Promise<requestDataResponse> {
    ...
    const calls = response.data.output?.filter(
        (m: OAIResponseOutputItem | OAIResponseOutputToolCall) => m.type === "function_call"
    ) as OAIResponseOutputToolCall[]
    const result: string = (
        response.data.output?.find((m: OAIResponseOutputItem) => m.type === "message") as OAIResponseOutputItem
    ...
    const reasoningTokens =
        response.data.usage?.output_tokens_details?.reasoning_tokens ??
