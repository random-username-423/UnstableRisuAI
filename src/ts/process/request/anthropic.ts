...
import { fetchNative, globalFetch, textifyReadableStream } from "src/ts/utils/fetch"
...
export async function requestClaude(arg: RequestDataArgumentExtended): Promise<requestDataResponse> {
    ...
    if (bedrock && aiModel !== "reverse_proxy") {
        ...
        const awsModel = !arg.modelInfo.internalID.includes("claude-v2")
        ...
        const encryptedThinking =
            hasEncryptedData && thinkingTokens > 0
                ? {
                      provider: "anthropic",
                      data: {
                ...
                const parseEvent = async (e: string) => {
                    try {
                        ...
                        if (parsedData?.type === "content_block_stop") {
                            ...
                            if (
                                parsedData?.content_block?.type === "redacted_thinking" &&
                                parsedData?.content_block?.data
                            ) {
