...
async function getAssetSrc(
    assetArr: string[][],
    name: string,
    assetPaths: { [key: string]: { path: string[]; ext?: string } }
) {
...
async function getClosestMatch(
    char: simpleCharacterArgument | character,
    name: string,
    assetPaths: { [key: string]: { path: string[]; ext?: string } }
) {
...
function wppParser(data: string) {
    ...
    lines.forEach((line) => {
        ...
        if (keyBracketStartIndex === -1 || keyBracketEndIndex === -1) throw new Error(`Invalid syntax ${line}`)
        ...
        const valueArray = line
            .substring(keyBracketStartIndex + 1, keyBracketEndIndex)
            .split(",")
            .map((str) => str.trim())

        // Validate Values
        for (let i = 0; i < valueArray.length; i++) {
            if (!valueArray[i]) throw new Error(`Empty Value in ${line}`)
        }
...
function blockStartMatcher(
    p1: string,
    matcherArg: matcherArg
): { type: blockMatch; type2?: string; funcArg?: string[] } {
...
export function setChatVar(key: string, value: string) {
    ...
    DBState.db.characters[selectedChar].chats[DBState.db.characters[selectedChar].chatPage].scriptstate["$" + key] =
        value
}
